...
- [Часть 2: Великое уравнивание](#часть-2-великое-уравнивание)
  ...
  - [Просперо](#просперо)
    - [SASL 83](#sasl-83)
    - [MIRANDA 84](#miranda-84)
    - [Miranda 0.292](#miranda-0292) 
    - [1984](#1984)
    - [Оксфордская галочка](#оксфордская-галочка)
    - [Mirandorwell](#mirandorwell)
    - [M-слово](#m-слово)
    - [Gotta go FAST](#gotta-go-fast)
    - [Самопроекция](#самопроекция)
    - [Частичная орбита](#частичная-орбита)
    - [Карри](#карри)
    - [ML Nouveau](#ml-nouveau)
      - [Haskell 0](#haskell-0)
      - [Pre-Haskell](#pre-haskell)
      - [Haskell 88.12](#haskell-8812)
      - [Haskell 89.05](#haskell-8905)
      - [Haskell 1.0](#haskell-10)
    - [Miranda Nouveau](#miranda-nouveau)
  - [Последние надежды](#последние-надежды)
    - [Algol 84](#algol-84)
    - [Scratchpad](#scratchpad)
    - [Переписывание по правилам](#переписывание-по-правилам)
      - [ASF+SDF, Stratego, Rascal](#asfsdf-stratego-rascal)
      - [Q, Pure](#q-pure)
    - [Алгебраическая спецификация](#алгебраическая-спецификация)
      - [OBJ](#obj)
      - [OPAL](#opal)
    - [Пустой список](#пустой-список)
    - [Чао, Пролог](#чао-пролог)
      - [Quintus Prolog](#quintus-prolog)
      - [SICStus Prolog](#sicstus-prolog)
...
Мы оставили в нулевой части рассказ об экспериментах Тернера над функциональными языками на создании KRC. Этот язык был радикальной попыткой очистить язык уравнений от лямбд, условных и `let/where` выражений, сохранив при этом первоклассную функциональность. Попыткой сделать из NPL функциональный язык другим, быть может более изящным способом, чем это сделали авторы HOPE.   
Была ли эта попытка успешной? Тернер использовал KRC в своих докладах о функциональном программировании [Turn81] [Turn82], когда хотел продемонстрировать все самое новое и лучшее. Но пользователи языков Тернера и многочисленные их переизобретатели и имплементаторы предпочитали скорее SASL, в котором победа над выражениями была не такой полной. Язык Августссона, похожий своими ограничениями на KRC, был временным переходным решением для создания не такого минималистичного языка.

### SASL 83

Тернер сделал шаг назад и вернулся к работе над предыдущим своим языком. Он добавил в очередную версию SASL, вышедшую в ноябре 83-го, ЦФ-выражения из KRC [Turn83].   

```
map f xs = {f x ; x <- xs}
```

ЦФ-выражения в этой, последней версии Тернеровского SASL могли быть компактнее современных. Например, можно было писать `{y <- ys <- xs ; x > 0}` вместо `{y ; y <- ys ; ys <- xs ; x > 0}` но все еще нельзя было описывать нетривиальные паттерны слева от `<-` и фильтровать списки по соответствию паттерну.   
Добавили ЦФ-выражения и в ARC SASL. Но там они выглядели почему-то иначе и как более современные, с квадратными скобками вместо фигурных и, по видимому, без сокращений. Возможно потому, что и были современнее, появились в языке позже, что мы не смогли установить, от ARC SASL осталось не очень много [Rich85].   

```
map f xs = [f x ; x <- xs]
```

В Университете Кента работа над имплементацией ранних языков продолжилась. Не позднее 84-го Саймон Крофт (Simon Croft) написал комбинаторный интерпретатор KRC, а Саймон Томпсон (Simon J. Thompson) в 87-ом написал компилятор SASL в категорные комбинаторы [Thom90]. Но эти работы продолжались уже без Тернера, который занялся другим проектом.   
Отступая из KRC, Тернер не смог забрать оттуда все лучшее. В SASL 83 не были перенесены гарды, которые плохо сочетаются с языком выражений. Условные выражения и `let/where` можно вкладывать друг в друга как угодно, а гарды - часть декларации функции и инструмент более "плоского" языка, которые не так просто объединить в одно выражение с другой частью декларации. Возможно поэтому гарды не состоялись и в более поздних версиях NPL/HOPE/SML. Вернее, спустя десятилетия, они в SML были добавлены, но это другая история.   
Борьба Тернера с выражениями в функциональных языках зашла слишком далеко. Но, может быть наоборот, она зашла недостаточно далеко?

### MIRANDA 84

Тернер решил, что недостаточно и продолжил борьбу. Для того, чтоб получить хороший минималистичный язык уравнений недостаточно только убирать привычные фичи, нужно что-то и добавлять. Но что? До сих пор Тернер создавал удачные и потому популярные комбинации уже придуманных кем-то другим и часто даже уже проверенных временем фич, хотя и с более легким и минималистичным синтаксисом. Но для решения KRC-проблемы не было ничего готового. И Тернер изобрел собственное решение, изменившее облик языков уравнений и вытеснившее выражение-`where`, существовавшее три десятилетия, со времен CPL и ISWIM. Тернер изобрел `where` как часть декларации. Правая часть каждого уравнения может состоять не только из выражения-результата и опционального выражения-гарда. Но и из опциональной группы деклараций, которые видны и в том и в другом.   
Исправив таким образом `where`-проблему, Тернер получил основу для "современного", ФЯ. В начале 80-х в KRC и SASL уже недоставало многого нового и лучшего. И для демонстрации нового и лучшего в ФП они подходили все меньше.   
Тернер начал HOPE-фикацию одним из первых, еще до появления версии NPL, называвшейся HOPE. Но начавшись рано, этот процесс все не мог набрать обороты. Планы по типизации SASL Тернер упоминает еще в 1977 [Turn79]. С тех пор прошло несколько лет, а планы все оставались планами. Видимо, избавившись от проблем с переменными в интерпретаторе, не особенно хочется возвращаться к ним в выводе типов.  
Со временем появились статья, полученная издательством в мае 84-го, в которой был KRC-псевдокод с сигнатурами типов [Meir85]. И типизированный KRC пришлось ждать не так долго, как типизированный SASL. Но дождались в итоге совсем не SASL и не совсем KRC.  
KRC, плюс `where` как часть декларации, плюс аннотация и вывод типов по Милнеру и абстрактные типы данных как в LCF/ML, алгебраические типы данных как в HOPE. Паттерн-матчинг не как в HOPE, а как в SASL и KRC для списков. Ну или как в Прологе. Со значимым порядком уравнений и нелинейными паттернами.
Нелинейные паттерны не были единственным отличием ПМ в языках Тернера от обычного современного ПМ в ФЯ. Сопоставление с единственным конструктором типа данных, например, такого вида `(a, b) = ...` соответствовало `~(a, b) = ....` в Haskell. К тому же - никаких `@`-паттернов.   
Как и другие заимствователи аннотаций типов из LCF/ML, Тернер выбрал только один из трех способов обозначения параметров. В этом случае - звезды. Тернер, по-видимому, просто не знал о других способах обозначения параметров в LCF/ML потому, что объясняет [Turn12] выбор звезд тем, что он не видел описаний ранних версий SML.   
В типизированном KRC-псевдокоде [Meir85] аннотации типов были еще более похожими на те, что в ML, а именно с тем же порядком конструктора типа и его параметра `Integer List`. В реальном типизированном "KRC" Тернер остановился на другом порядке конструкторов и параметров - как в HOPE. Но `::` вместо `:` для аннотаций типов, появившийся еще в KRC-псевдокоде потому, что `:` уже занят для `cons` [Turn12], таким и остался в реальном  Тернеровском языке с типами. Возможно, что важную роль сыграло то, что аннотаций типов в коде ожидалось меньше, чем конструкторов списков. Тернер считал, что опциональность аннотаций - как в ML и не как в HOPE - важный плюс [Turn84]. Позднее, правда, рекомендовал писать аннотации для всех функций верхнего уровня [MirMan].   
Добавив в KRC все недостающие до полной HOPE-фикации составляющие Тернер получил в результате свой magnum opus - язык Miranda.   
...
Работа над имплементацией Miranda началась, судя по отметкам в исходном коде [MirSrc] не позднее августа 83-го. Имплементация была все тем же комбинаторным интерпретатором. Который теперь использовал опыт специальных машин вроде ARC NORMA. А именно разворот указателей и B1-комбинатор Марка Шивела. Тернер пытается улучшить производительность добавлением больших комбинаторов. Правда, не генерирует специальные версии как делали авторы LML и Ponder. Такие функции как `map` и `foldr` не написаны на Miranda в библиотеке, а добавлены в стандартный комбинаторный набор интерпретатора вместе с `S`, `K` и так далее. Но даже усовершенствованный комбинаторный интерпретатор - имплементация не из быстрых.  
Судя по отметкам в том же исходном коде, первый код на Miranda был написан не позднее января 84-го.   
Miranda впервые была продемонстрирована за пределами Университета Кента, по всей видимости, во время доклада Тернера в Лондонском королевском обществе 15 февраля 84-го года [Turn84].   
Доклад был не о Miranda, а об исполняемой спецификации, но для нашей истории важнее самое раннее описание языка, настроение Тернера и реакция слушателей доклада.   
В ранней Миранде продолжаются начатые в KRC эксперименты с нотацией для конструирования списков. ЦФ-выражений аж два вида: абстракция множеств - с фигурными скобками как в KRC и SASL 83, и абстракции списков с квадратными скобками как в ARC SASL (может быть это в ARC SASL как в Miranda, конечно). Абстракция множеств убирает дублирующиеся элементы как функция `nub`, то есть является ленивой версией ЦФ-нотации из NPL. Производит результат типа список, а не множество.   
Абстракция списков работает "диагонально" как в KRC и SASL 83 и позволяет некоторые из тех же сокращений.   
Тернер похоже уже не особенно верил в возможные успехи специального железа, но еще не верил в успехи имплементации ленивых ФЯ для обычного железа. Специальное железо для него пока что выглядело убедительнее.   
Считал, что по крайней мере можно будет точно использовать чистые ФЯ для исполняемых спецификаций и прототипирования. Пока не ясно, заключал Тернер, подходят ли "современные ФЯ" для написания серьезных программ, хотя кое-какие результаты уже есть. Опыт Лиспа тут бесполезен - не имеет отношения к "современным" ФЯ.   
Знакомые по первому описанию HOPE надежды на кое-как исполняющуюся спецификацию, но не в 80-ом, а в то время, когда, как мы выяснили в предыдущей части, функциональные языки научились имплементировать уже более-менее быстрыми. И даже уже успели кое-кому рассказать об успехах.   
К докладу прилагается стенограмма его обсуждения.   
Наш знакомый по предыдущей части, автор и имплементатор языка Ponder Джон Фейрберн не доволен типами в Miranda. Особенно тем, что сравнение можно применять к значениям любых типов и в случае неподходящих, вроде функций - сравнение приведет к ошибке времени выполнения. Как в OCaml сегодня. Фейрберн предлагает передавать функции сравнения в функции это сравнение использующие.   
Но Тернер отвечает, что это неудобно. И ничего лучше система Милнера не позволяет. И ничего лучше, чем система Милнера пока не прошло проверки временем. Система Милнера относительно простая и делает более-менее то, что надо Тернеру. Сойдет и так!   
Падучие функции не выглядят подходящими для написания спецификаций? Не беда, просто считаем падение и зацикливание специальными значениями - и вот функции уже везде определены. От падений все равно никуда не денешься, они возможны на каждом шагу.   
Тернер перенес в типизированный язык много удобств из своих предыдущих, гораздо менее типизированных языков. Тип чисел один - и целые числа или с плавающей точкой определяется во время исполнения исходя из того, что с ними делают. Все можно не только сравнивать, но и превращать в строки и читать из строк. Строки получаются интересными не всегда и чтение из них не всегда заканчивается благополучно.   
Еще один слушатель доклада - Вадлер не согласен с плохим отношением Тернера к Лиспу вообще, но согласен, что языки Тернера (Вадлер писал на KRC) требуют писать намного меньше кода, чем Лисп. Вадлер не считает, что ЦФ-выражения играют важную роль в этом. Прочие инновации современных ФЯ и ленивость - важнее. Вадлер заявляет, что предпочитает не пользоваться ЦФ-выражениями, использует вместо них библиотечные комбинаторы вроде map.   
Оба этих заявления довольно неожиданны в свете дальнейшей деятельности Вадлера.   
Эта первая описанная версия Миранды не была пригодна для использования еще почти полтора года.

### Miranda 0.292

Только в мае 85-го была готова альфа-версия для внутреннего использования в Университете Кента.   
Но язык стал более-менее широко известным в узких кругах функциональных программистов после доклада о нем на очередной конференции по функциональным языкам и компьютерным архитектурам в сентябре 85-го [Turn85].   
В следующем месяце, в октябре 85-го появилась публичная бета 0.292.   
В последнее несколько лет мы видели немало плодотворной работы над функциональными языками, пишет Тернер. Хотя не на все важные вопросы уже найдены ответы, пришло время сделать полученные результаты доступными для более широкого сообщества. Доступными в форме стабильных имплементаций с хорошей документацией.   
Тернер признает, что до сих пор много разногласий по принципиальным вопросам, так что сделать единый функциональный язык не получится.   
Miranda основана на SASL и KRC. К этому добавлены средства для работы с большими проектами: типы и модули. Это похоже на HOPE. Принципиальная разница в том, что Miranda - ленивый язык. По этой причине более консистентный и выразительный.   
Miranda также и менее многословный язык. Требует меньше аннотаций и синтаксис - легче.   
Как и в других похожих случаях, например, в случае SML, после заявления о том, что эксперименты закончены, следуют эксперименты.   
В новой версии Miranda нет абстракций множеств. Удалять дубликаты из списков предлагается библиотечной функцией.   
Но зато появляется два вида абстракций списков. Тому синтаксису, который был в Miranda 84 теперь соответствует более привычный сегодня конструктор списков.   
...
У "диагонального" построителя списков другой синтаксис.   
...
`i,j <- xs` это сокращение для `i <- xs; j <- xs` как в KRC, не перепутайте с `(i,j) <- xs` , что просто паттерн слева от `<-`. Паттернов слева от `<-` в KRC не было, но в Miranda все работает более-менее ожидаемо.   
...
Пропускать тривиальное выражение, ссылающееся на результат одного генератора больше нельзя. Такое определение `filter p xs = [x<-xs;p x]` больше не работает, так что записать "быструю" сортировку так же коротко, как в SASL 83 уже никогда не получится.   
Но сходство получающихся абстракций списков с современными пока еще обманчиво. В них еще есть, например, формы генераторов, позволяющие писать так:

```
fibs = [ a | (a,b) <- (1,1), (b,a+b) .. ]
```

Так или иначе, эксперименты с абстракциями списков приблизили их к современному виду. Об экспериментах Тернера над другой знакомой фичей такого не скажешь.   
Алгебраические типы в Miranda, на первый взгляд выглядели знакомо. Для многих читателей, полагаем, даже более знакомо, чем АлгТД в HOPE. В HOPE у конструкторов всегда одно поле, которое может быть туплом. Но в ленивом языке, таком как Миранда, конструктор с несколькими полями не то же самое, что конструктор с полем-туплом, и есть смысл в первоклассных конструкторах со многими полями. Поля могли быть строгими, для указания этого существовала `!`-аннотация.   

```
list * ::= Nil | Cons *! (list *)!
```

Обратите внимание, что имена конструкторов (но не конструкторов типов) должны начинаться с заглавной буквы. Это требование появилось не сразу, в первых описаниях языка его нет.   
В Miranda АлгТД декларируются без ключевого слова, специальным оператором. Что максимально сближает их с BNF [Turn12]. Нотацией, на основе которой Кнут с Хоаром и придумали этот способ описания АлгТД.   
Можно объявлять типы без конструкторов, но тут оператор вместо ключевого слова не позволяет так просто не написать конструкторы. Так что тип без конструкторов объявляется так `foo :: type`.   
Название "алгебраические типы" использовал и популяризировал как раз Тернер. В других языках их обычно называли "конкретными типами" или просто "типами данных".
Такой алгебраический взгляд Тернера имел последствия, которые не закончились на названии. По крайней мере пока. Миранда сделала движение к своим пред-NPL корням - языкам алгебраической спецификации. Что если разрешить описывать не только свободную алгебру? - задумался Тернер. И разрешил:
...
Такой список упорядочивается при конструировании. Это реализуется как "умный конструктор" такого вида:
...
Этот эксперимент Тернер посчитал неудачным, как и аннотации строгости для полей. Что, наверное, более неожиданный поворот истории. Обе фичи были исключены из языка в 89-ом году. Но остались имплементированными для совместимости.
Но до этого урезания пока было еще далеко. В 86-ом, в очередном описании языка [Turn86] Тернер пишет, что на момент описания прототип работает на важнейших для нашей истории платформах 80-х VAX и SUN, используется в 70 университетах и лабораториях. Всего у бета-версии таких пользователей было 88.   
Первый релиз - 1.009 - был готов только в апреле 87-го. Новой версии имплементации соответствовала и новая версия языка. И в создании этой версии Miranda поучаствовали и другие важные герои нашей истории.   

### 1984

Филип Вадлер получил степень бакалавра по математике от Стенфорда в 77-ом, степень магистра компьютерных наук от университета Карнеги-Меллона в 79-ом, защитил докторскую диссертацию там же в 84-ом. За это время Вадлер стажировался в стенфордской лаборатории искусственного интеллекта, исследовательском центре Xerox в Пало-Альто и лабораториях Bell. Под руководством важных героев нашей истории, например Джона МакКарти и Джеймса Морриса.   
В 83-87 годах Вадлер работал в Оксфорде в исследовательской группе по программированию. Где принял некоторое участие в разработке Standard ML. Но более важной была его работа над другим языком [Wadler].   
Ричард Берд начал работать в Оксфорде в том же году, что и Вадлер, но принадлежал скорее к предыдущему поколению Дарлингтона и Гордона. Защитил диссертацию в начале 70-х [Gibb22].   
Как вы помните, Тернер долго не выпускал публичную версию Miranda. И Вадлер, познакомившийся с этим языком одним из первых, еще в начале 84-го, устал ждать. Мы уже не раз упоминали (и еще упомянем) многочисленные воспроизведения предыдущих языков Тернера, пришло время и воспроизведения Miranda. Свою версию Вадлер успел закончить, по крайней мере в первом приближении, даже раньше Тернера. Хотя, если судить по названию, можно предположить, что не так скоро, как планировал. Первое описание [Wadl85] языка Orwell и как-то работающей его имплементации для VAX-11 было готово первого апреля 85-го года. На месяц раньше Miranda 0.292.   
Понятное дело, что описание содержит обширные цитаты из Оруэлла о влиянии языка на то, что на нем думают. Также содержит цитаты Дональда Кнута из статьи [Knut74] в которой тот сомневается во вредоносности `goto` и шутит про языки будущего UTOPIA 84 и NEWSPEAK.   
Вадлер разрабатывал язык Orwell не один. Ему помогали Ричард Берд, получивший от директората Алви деньги на разработку языка [Bird86] и наши старые знакомые по первой части и истории ML соответственно Хьюз и Суфрин. Первым пользователем был Джереми Джейкоб (Jeremy Jacob). Ко всем ним, кроме Хьюза, Вадлер рекомендует обращаться с вопросами о языке, если сам Вадлер недоступен. Вадлер перечисляет и языки из которых он черпал идеи: Iswim, NPL, Hope, ML, SASL, KRC, Miranda.  
То, что Вадлер успел раньше Тернера объясняется не только способностями его и его оксфордских коллег, но и тем, что Orwell 1.0 в значительной степени менее доделан, чем Miranda 0.292. Например, не имплементирована проверка типов. До начала выполнения. В случае несовпадения типов программа все же не умрет молча, но сообщит об ошибке. Интерпретатор Orwell делает это не самым обычным образом    

```
[3 + 4, 3 + 'a', 3 + (2 DIV (3 - 3))]
```

вычисляя все что может и печатая вызвавший ошибку код, когда не может. Вот так:

```
[7, {3 + 'a'}, {3 + {2 DIV 0}}]
```

Вадлер не стал делать один тип и для целых чисел и для чисел с плавающей точкой, как делал Тернер. В языке Вадлера пока что были только целые числа.   
Но не все отличия между языками Вадлера и Тернера - недоделки.   
В соответствии со свежими идеями Дональда Кнута, в Orwell-программах текст по умолчанию является комментарием, это код требует указания. Придуманным Ричардом Бердом способом - с помощью таких вот `>` галочек.  
Orwell в большей степени NPL, чем любой из языков Тернера. NPL даже упомянут как один из повлиявших на Orwell языков, что случается не часто. И Orwell похож на NPL не только из-за недоделанной проверки типов в сочетании с возможностью объявлять конструкторы. В Orwell как в NPL есть n+k паттерны:   
...
и последовательность уравнений с паттернами не должна иметь значение, так что пересекающиеся паттерны запрещены. Так писать нельзя:
...
Но, как выяснилось, программисты хотят писать такой код. Поэтому, писать так нельзя только по умолчанию. Есть ключевое слово, которое позволяет сделать порядок уравнений с паттернами имеющим значение, как в языках Тернера, Прологе и SML:
...
синтаксис гард тяжелее, чем у Тернера
...
И в `WHERE` нельзя объявлять функции, имена с параметрами и вкладывать другие `WHERE`. Это, в основном, только эстетическая проблема в HOPE, в котором есть лямбды, так что параметры просто переезжают из левой части уравнения в правую. Но в Orwell лямбд нет, как их нет и у Тернера.  
После критики, с которой Вадлер обрушился на ЦФ-нотацию во время обсуждения первого доклада Тернера о Miranda довольно неожиданно, что Вадлер имплементировал ее в своем интерпретаторе Orwell раньше проверки типов. И даже написал об имплементации ЦФ-нотации главу для книги Пейтон-Джонса [SPJ87]. Вадлер к тому же изобрел современное название для ЦФ-нотации - list comprehensions.   
Современное у них не только название, но и вид. Генераторы разделяет запятая, а не точка с запятой. Это может быть тоже влиянием NPL, там генераторы разделяла запятая, хотя между генераторами и условиями-фильтрами там был другой разделитель. Скобки квадратные (тут, к счастью, виляния NPL не было) и результаты вычисления не диагональные. Правда, одно сокращение еще работает, можно писать `[u <- xs | u < x]` вместо `[u | u <- xs, u < x]`, так что можно еще написать и самый лаконичный "квиксорт"
...
В Orwell можно определять операторы. Но это более-менее типично для ФЯ, отсутствие таких возможностей в языках Тернера - наоборот нетипично.   
...
Следующее описание языка вышло в декабре того же, 1985-го года. Но похоже, что до нас оно не дошло. Известно, что оно было - из списка всех редакций [Wadl90]. Сохранилось следующая редакция от сентября 86-го года [Wadl86]. Отсканированная версия не содержит даты, но ее можно установить по ссылкам в статье [Runc91].   
Эта третья версия Orwell называется не Orwell 3, а OL (Oxford Language). Вместе со старым названием из описания исчезли и цитаты Оруэлла и Кнута.   
Вадлер имплементировал интерпретатор и пользовательский интерфейс. На эту работу Вадлер получал грант от ICL. Но теперь над имплементацией работает Квентин Миллер (Quentin Miller). Интерпретатор Orwell теперь работает на рабочей станции SUN с процессором MC68K.   
Много чего недоделанного в прошлых версиях теперь доделано. Теперь есть не только целые числа, но и числа с плавающей точкой как отдельный тип и с называющимися иначе, не перегруженными операторами, как в OCaml сегодня. Для облегчения частичного применения операторов появились сечения с современным синтаксисом. Тернер приписывает изобретение сечений Ричарду Берду и Дэвиду Уайлу (David Wile) [Turn12].   
Главное улучшение, конечно, проверка типов и их вывод и опциональная аннотация. HOPE-фикация успешно завершена! Параметры типов, к счастью, не выглядят как в Miranda. Вадлер не стал выбирать один из способов именования параметров типов из LCF/ML. Не сделал он параметры и объявляемыми как в HOPE. Параметризованные типы выглядят как функции. Конструкторы типов не параметризуются, так что начинать их имена с заглавной буквы не обязательно. Проверку типов имплементировал Мартин Расковски (Martin Raskovsky).   
...
В `WHERE` теперь можно объявлять функции и даже со вложенными `WHERE`. Так что язык Вадлера получил полноценный Тернеровский `where` и окончательно мирандизировался. В описание тем временем добавлено уточнение, что не все из перечисленных еще в первом описании языков повлияли на Orwell/OL в равной степени. И два которые повлияли больше прочих - это KRC и ML. Конечно, конечно.  
Вадлер пока не считает язык законченным: нет ввода-вывода, еще нет модулей. В Miranda модули есть.   
Но расширениями дело не ограничилось. Orwell - достойный наследник NPL и в том, что каждое дошедшее до нас описание содержит массу мелких изменений.   
Построители списков с одной стороны менее похожи на современные. Вместо `,` как разделителя генераторов теперь Тернеровский `;` разделитель. С другой стороны убрали последнее сокращение, так что размер "квиксорта" теперь не меньше современного.   
...
Изменился синтаксис объявления упорядоченных уравнений с ПМ. Теперь ключевое слово отделяет неупорядоченную секцию уравнений от той, что срабатывает если они не сопоставились.   
...
Ключевое слово для объявления АлгТД теперь `DATA`, а `TYPE` теперь используется для объявления синонимов.   
...
Читатели, знакомые с Haskell, думаем, уже узнали в Orwell/OL многие детали. Часто странные, не очень популярные (за исключением сечений) детали, которые выделяют Haskell среди прочих ФЯ. Но добавление этих особенностей в другие языки произошло еще до появления Хаскеля. Некоторые из них Тернер добавил в Miranda перед релизом версии 1 (точнее версий 1.009, 1.016 или 1.019 в зависимости от платформы, для которой писали имплементацию) в апреле 1987-го [MirMan]. Но почему Тернер сделал это? Что заставило его оруэллофицировать свой язык, хотя до сих пор предпочитал удалять из своих творений все лишнее и даже не такое уж и лишнее.   
Дело в том, что Берд и Вадлер работали не только над функциональным языком, но и над учебником "Введение в функциональное программирование" [Bird88]. Тернер следил за его созданием, давая авторам советы и, по видимому, правильно оценил его потенциал. И по причинам, к которым мы еще позднее вернемся, каждый шанс повысить популярность Miranda был важен для Тернера. Так что Тернер, Берд и Вадлер договорились, что примеры в книге будут на псевдокоде, близком к общему подмножеству Orwell и Miranda. И не смотря на то, что Orwell уже не особенно отличался от подмножества Miranda, некоторая доработка была необходима. Авторы книги, конечно же, хотели использовать свои любимые фичи, так что Тернеру нужно было добавить их в Miranda.   
И Тернер добавил в свой язык сечения, но не пользовательские операторы. Вернее, не операторы с небуквенно-цифровыми именами. Любую функцию с подходящим числом параметров в Miranda можно было использовать инфиксно добавив к имени `$`, как сегодня в Хаскеле можно сделать добавлением других символов.   
Тернер добавил "литературные" комментарии, правда, оставив и комментарии обычные. Добавил Тернер и `n+k` паттерны. С ними он был знаком еще по NPL, но посчитал сначала, что для целых чисел они подходят не так хорошо, как для натуральных. Единственной разновидности чисел в NPL. Но Берд хотел `n+k` паттерны и получил их [Turn12].   
Самое неожиданное нововведение - слияние Тернеровских KRC-образных гард из альфы и беты Миранды 
...
в комбинацию, смело объединяющую все худшее из обеих подходов:   
...
Тернер, правда, оставил имплементацию старых гард и разрешил их использовать.
Orwell тем временем вернул себе первое имя, все соответствующие цитаты и шутки в описании [Wadl90] и в еще большей степени мирандизировался. До нас дошло описание Orwell 6 января 90-го года, не предыдущие два. Но до нас дошел код [Jone90], написанный на мирандизированном Orwell в 89-ом году, так что окончательная мирандизация должна была произойти во времена Orwell 4 (декабрь 87-го) или Orwell 5 (октябрь 88-го). Скорее раньше, потому что позже у авторов Orwell было чем заняться и без очередной переделки Orwell. Но об этом позднее.   
В этот раз в описании как языки, повлиявшие на Orwell в наибольшей степени, обозначены KRC и Miranda. И в этот раз сложно не согласиться с авторами описания.      
Встречные усилия Вадлера и Берда по установлению общего подмножества с Miranda заключались в едином типе для чисел вместо отдельных целых и чисел с плавающей точкой, идентичном с Miranda синтаксисе объявления типов. Гарды Orwell заменили на компромиссные гарды, а отсутствующие в Miranda ключевые слова превратили в прагмы. Синтаксис для прагм в Miranda был.   
Например, стало прагмой ключевое слово для разделения секций уравнений, порядок которых важен и тех, что не важен:   
...
Да, от паттерн-матчинга как в NPL/HOPE в Orwell не отказались, но это не проблема - можно писать в книге такой код, который сработает в обоих языках.   
Стали прагмами и ключевые слова для объявления пользовательских операторов. Которые в Miranda не приняли и от которых в Orwell не отказались.   
...
Как видите, об одинаковом синтаксисе для параметров типов тоже не договорились. И там и там они остались прежними. Это несоответствие обошли так: в книге параметры обозначены греческими буквами, программист на Orwell должен был их заменять латинскими буквами, а программист на Miranda - звездочками.   
Тернер выпускал не только описания, у Miranda было открытое бета-тестирование и пара релизов. А что же Orwell/OL? Остался ли он только внутренним Оксфордским языком для обучения функциональному программированию?   
17 октября 90-го года, Квентин Миллер опубликовал в рассылке comp.lang.functional анонс: имплементация Orwell для SUN-3 и SUN-4 скоро, в январе 91-го, будет доступна всем желающим бесплатно. Доступен будет и исходный код интерпретатора на C [Mill90a].   
И уже 26 ноября того же года Миллер написал следующее сообщение: извините, анонсированное распространение имплементации Orwell не состоится из-за "юридических осложнений" [Mill90b]. Ох.   

### M-слово

И в 87-ом году, году первого релиза Miranda, уже имелись какие-то результаты работы над компиляторами Miranda. Над компиляторами работал не Тернер, а знакомые нам по предыдущей главе авторы и имплементаторы Clean. Этот Clean - промежуточный язык. И первоначально промежуточный между машкодом VAX и тернеровскими языками SASL и Miranda.   
Участники Голландского спец-машинного проекта пишут [Bare87] [Brus87], что разрабатывали два компилятора Miranda, которые генерируют Clean код. Один на Modula-2 и другой на Miranda. Трансляция Miranda в Clean описана с примерами на Miranda в книге [Plas93].   
Но позднее акцент с имплементации Miranda смещается на развитие CLEAN в полноценный, большой ФЯ. А компиляторы Miranda становятся "программой для трансляции Miranda в Clean" [Nock91]. В исходном коде компилятора Clean остались только закомментированные следы Miranda-фронтенда.   
Очевидно, что в этот раз имплементация языков Тернера натолкнулась на сложности. Почему?   
Дело в том, что Тернер не хотел, чтоб Miranda имплементировали. Miranda - зарегистрированная торговая марка, а имплементация этого языка - коммерческий продукт.   12 апреля 83-го года Тернер со своей женой Уной (Una Turner (Williams)) основали компанию Research Software Limited [RSL]. Компания эта продавала интерпретатор Миранды написанный Тернером. Обычно университетам и реже лабораториям.   
В чем заключалась борьба Тернера с имплементаторами Миранды? Лисперы пишут о своей борьбе с остальными лисперами много и с удовольствием. Поэтому в нашей истории не-Лиспа относительно много таких историй. Участники Эдинбургской исследовательской программы, наоборот, не особенно любят выносить сор из Эдинбургской избы. Но американские участники программы, которых становилось больше и больше, не так последовательно следуют этому принципу, как, например, британские. Как мы уже видели на примере Полсона и МакКвина. И еще увидим.   
И, благодаря американцам Худаку и Вадлеру, до нас дошли обсуждения конфликтов Тернера с имплементаторами Миранда-образных языков в рассылках.   
Худак походя отмечает, что из за "проблем" с Тернером задерживалась книга Пейтон-Джонса по имплементации ФЯ. Имплементации Миранды, в основном. Не известно каких именно проблем, и сам Пейтон-Джонс не стал развивать эту тему. Вадлер, автор нескольких глав в этой книге задержку подтверждает.   
С книгой Берда и Вадлера проблем, правда, не было. Проблема была в другом.   
Вадлер рассказывает о том, как он, Берд и Тернер сначала совместными усилиями сделали Orwell и Miranda максимально похожими. Чтоб их можно было изучать по одному учебнику Берда и Вадлера. После чего, к удивлению Вадлера и Берда, Тернер стал настаивать, что распространять Orwell за пределами Университета Оксфорда нельзя. Слишком похож на Миранду!   
Понятно, почему Тернер не хотел появления конкурирующих имплементаций Миранды. Если судить по названию "Miranda" - можно предположить, что он чем-то был уже недоволен еще до коммерциализации своих усилий по созданию ФЯ. На официальном сайте объясняется [MN], что язык назван именем персонажа "Бури". Там, правда, не упоминается, что "Буря" - это история мести и Миранда - одно из средств осуществления этой мести. Мести, правда, более близкой не к обычной шекспировской, а к выражению о том, что лучшая месть - это успех. И успех более-менее состоялся. 200 университетов и 40 коммерческих компаний купили [Turn12] неизвестное нам количество лицензий.   
Бизнес был, по всей видимости, относительно успешным. Но успех все же был ограниченным по ряду причин. Неамбициозная имплементация вероятно позволила Research Software Ltd. оставаться семейным бизнесом с минимальными расходами. Но и свободную замену в академии было разработать проще. Проще, чем для коммерческого SML и Common Lisp.   
Неамбициозность имплементации не ограничивалась тем, что Тернер перестал экспериментировать с имплементациями ФЯ, хотя этими работами он обычно и был известен многим важным героям нашей истории. Инструментарий для пользователя тоже был сделан без особых амбиций.   
Имплементаторы первых ФЯ или старались имплементировать продвинутый REPL, используя "нижележащую" Лисп-систему (LCF/ML, ALFL) или имплементируя интерактивный компилятор самостоятельно (VAX ML, Poly). Или, как авторы LML, даже и не пытались имплементировать REPL, писали компилятор, который нужно было использовать как компилятор C, с помощью UNIX-инструментария вроде `make`.   
Тернер нащупал третий путь. Минимальный REPL не позволяет ничего объявлять. Команда открывает загруженный файл с исходным кодом в редакторе `vi` (по-умолчанию, можно сконфигурировать для другого редактора). Выход из редактора перезагрузит файл в мини-REPL. Если в файле есть ошибки, то при следующем открытии в редакторе файл будет открыт на строке с первой ошибкой. Такая интеграция редактора с интерпретатором дожила до 2000-х годов в виде HUGS и старых версий ghci. Эти мини-REPL-ы, правда, не были настолько минимальными. Мини-REPL может исполнять только выражения, но в языке Тернера язык выражений намного беднее того, что в Хаскеле. Единственное более-менее выразительное выражение - это ЦФ-нотация и пользователи в рассылках рекомендовали ее использовать для эмуляции `let in`. При том, что в Тернеровских ЦФ-выражениях, в отличие от современных list comprehensions, `let` нет.   
Другими словами, инструментарий был не особенно богат фичами. Но как-то работал и пользователи это ценили.   
Желание Тернера предотвратить альтернативные имплементации Миранды, конечно же, натолкнулось на желание многих Миранду имплементировать. Что их не устраивало в имплементации Тернера?   
Miranda исполняется очень медленно. Сохранилось множество сравнений производительности основных версий интерпретатора Miranda с имплементациями прочих ФЯ. Как правило - не в пользу Миранды.   
Голландские компиляторы Miranda через CLEAN генерировали код, работающий в 30-50 раз быстрее, чем Miranda 0.292 [Brus87] [Bare87]. Бета версия интерпретатора Миранда в 20-30 раз медленнее, чем CLEAN того же года. В 5-6 раз медленнее, чем голландский компилятор SASL, написанный Риет Олман (Riet Oolman). В 165-190 раз медленнее, чем Lazy ML.   
Miranda 2.009 В 96 раз медленнее LML и в 410-476 раз медленнее, чем CLEAN [Koop90].    
В 523-888 раз медленнее компилятора Scheme [Feel94]. Все еще не худший результат Миранды в таких сравнениях. Худшим был, по всей видимости, "результаты не приведены потому, что интерпретатор Miranda не закончил работу за разумное время" [Nock91].   
Значительный вклад в эти впечатляющие результаты делает объединение Тернером целых чисел и чисел с плавающей точкой в один тип. Но, как видите, это не особенно помогло Миранде соревноваться с некоторым имплементациями "динамических" языков.   
SK-интерпретатор Тернера вдохновил многих героев этой истории на создание существенно более производительных имплементаций. Но, даже усовершенствованный техасскими ФП-машинистами, SK-интерпретатор все еще оставался имплементацией 70-х. Это, к сожалению, означало не только низкую скорость исполнения. Интерпретатор может компенсировать эту проблему более скромными системными требованиями. Но не интерпретатор ФЯ 70-х. SK-интерпретатор в каком-то смысле сочетает недостатки обоих подходов. Трансляция из лямбды в комбинаторы создает развесистые деревья в куче и требует почти такой же мощной машины, что и компилятор. Это ПО для миникомпьютеров и дорогих рабочих станций, а не относительно дешевых ПК.
Существуют сравнения скорости компиляции [Hart96] в которых интерпретатор Миранды требует больше памяти чем компиляторы Clean и Poly/ML.   
Первый интерпретатор Orwell имел похожую имплементацию и, следовательно, те же недостатки. Но следующие имплементации Orwell их исправили и стали гораздо более опасными конкурентами Миранды. Но это уже другие истории.   
Имплементация Miranda Тернера - это, вероятно, самая медленная имплементация ФЯ. По крайней мере самая медленная имплементация ФЯ, которую можно было купить за деньги. Кстати, о деньгах.   
Потенциальные покупатели в рассылках писали, что не особенно довольны стоимостью лицензии в $625 ($1800 в 2026) на один процессор.   
Вадлер вспоминает, что покупка этих лицензий была проблемой для Оксфорда и одним из важных мотивов использовать для преподавания Orwell. До того использовался бесплатный KRC так что известно из опыта - использование функционального языка изобретенного не в Оксфорде было допустимым.   
Цена сопоставима с Poly/ML - коммерческим продуктом параллельной SML-ветви Эдинбургской программы. Настоящим компилятором и богатым фичами настоящим REPL.   
Но, не смотря на множество недостатков имплементации Миранды по сравнению с Poly/ML, у нее есть существенное достоинство, которому сложно что-то противопоставить. Интерпретатор Тернера имплементирует язык Тернера, а не ML. И многие функциональные программисты 80-х хотят писать на языке Тернера.   
И если Тернер и сам не хочет делать современный компилятор и другим не дает, то у функциональных программистов найдется на это ответ. Одно из обсуждений такого ответа состоялось в начале августа 91-го в рассылке и архив этой рассылки сохранился.   
Допустим, Miranda - защищенная торговая марка и может быть Тернер может создать какие-то сложности для того, кто разрабатывает и публикует "компилятор Miranda". Худак подозревает, что все, кто выполняли требования Тернера, делали это только чтоб не обидеть Тернера. Худак утверждает, что никому не удавалось защитить язык программирования от имплементации третьими лицами. Adobe попыталась с Postscript - безуспешно. Деталей Худак, правда, не знает. Мы не разбираемся в юридических вопросах и истории Adobe в достаточной степени для того, чтоб прокомментировать насколько эти представления верны. Отметим только, что такие представления были у функциональных программистов того времени, и для нашей истории это как раз и важно.   
Нихил, один из основных разработчиков языка Id, формулирует позицию так: имплементировать точную копию Miranda под другим названием было бы непрофессионально и неэтично. Но не нелегально. Тут Тернер полностью бессилен. И в случае языка, который от Miranda чем-то отличается - нет и этических проблем. Это нормальный процесс развития языков программирования.   
И несколько имплементаторов Миранд следовали этой формуле - вносили кое-какие изменения в Миранду и называли не-Мирандой. Тем более, они и так собирались в Miranda что-нибудь добавить. Не все любили минимализм так же сильно как Тернер.   

### Gotta go FAST

Один из проектов по созданию (помимо прочего) быстрой Миранды так и назывался FAST (Functional Programming for Arrays of Transputers). Работы велись в 1990-93 годах.   
Участниками были Питер Хертель (Pieter H. Hartel) уже поучаствовавший в голландском параллельном проекте и в нашей истории, Хью Глейзер (Hugh Glaser) из университета Саутгемптона, наш старый знакомый по истории HOPE и ALICE-машины Майк Рив с коллегами Полом Клеил (Paul Kelly) и Стюартом Коксом (Stuart Cox) из Имперского Колледжа Лондона и Джеймс Коуни (James Cownie) из Meiko.   
В проекте FAST работали над набором трансляторов из одних ФЯ в другие и оптимизирующим компилятором в C [Hart93].   
Был ли FAST действительно быстрее Миранды? Был быстрее в 105 раз [Hart96].   
Для многочисленных промежуточных стадий во всех этих трансляциях и компиляциях разработали два промежуточных языка, которые так и назывались. Первый - Basic Intermediate - простой ленивый ФЯ, более-менее похожий на SASL до победы Тернера над `let in`. Ожидаемо для промежуточного, язык был минимальным, без АлгТД, ЦФ-нотации, числовых последовательностей, синтаксиса для туплов, возможности применять функции инфиксно.   
Второй язык был совсем другим. Advanced Intermediate был большим подмножеством Miranda, с АлгТД, list comprehensions, числовыми последовательностями и прочими вещами, которых обычно не ожидают в промежуточных языках. Сначала язык был меньше, но со временем оброс фичами как CLEAN. Но если CLEAN обрастал обычными HOPE-фичами с узнаваемыми оригинальными деталями, Intermediate стал почти-Мирандой в соответствии с доктриной Нихила.  
Для того, чтоб быть Мирандой языку недоставало единого типа для целых чисел и чисел с плавающей точкой, синтаксиса отступов, средств раздельной компиляции, полиморфного сравнения. Сравнения определены только для некоторых примитивов, писать их для АлгТД нужно самому. Описания абстрактных типов принимаются, но их абстрактность игнорируется.   
К этой не-Миранде, как полагается, и что-то добавили. Массивы, комплексные числа, раздельные типы для целых и чисел с плавающей точкой. Вместо отступов - точки с запятой. В языке есть `LET IN` выражение. Ключевое слово для директивы инлайна. Для полиморфных типов определены операции рефлексии, можно проверить не являются ли значения значениями некоторых примитивов.   
...
LET y = 1 IN map (+ y) [1..3];
```

Язык Intermediate не особенно похож на промежуточный? Но технически он им являлся. Пользователи FAST писали на Miranda, используя инструментарий этого языка, потом транслировали Миранда-код в Advanced Intermediate sed-скриптом. В самом Intermediate не было даже нормальной проверки типов с сообщениями об ошибках для программиста. Вывод типов происходил только в оптимизаторе.  
Advanced Intermediate транслировался в другие ФЯ, например CLEAN и LML. Но был и оптимизирующий бэкенд, компилирующий через C.  
К истории FAST мы еще вернемся, когда будем подробнее рассматривать историю компиляции функциональных языков через C. К тому же, Intermediate не единственная не-Миранда, которую FAST компилировал.
Пара других не-Миранд компилировалась не через C, а через Scheme, так что самое время посмотреть как-там идут дела с имплементацией Схем.   

### Само-проекция

Кент Дибвиг (R. Kent Dybvig) приобрел опыт, работая над рядом имплементаций Scheme [Dybv06].   
Первым опытом имплементации диалекта Лиспа для него стала работа над многопоточной Scheme SDP в 80-81 годах вместе с Рексом Дуайером (Rex Dwyer) в Университете Индианы. SDP написана на Simula и использовала рантайм этого языка со сборщиком мусора. Так что обсуждаемая в нулевой части Simula может и не стала популярной основой для имплементации ФЯ, но была вполне реальным кандидатом.   
От этих экспериментов у Дибвига остался только опыт, но все следующие его имплементации использовали сколько-нибудь общего кода или, хотя-бы использовались для разработки следующий и объединены в семейство имплементаций, которое разрабатывается и сегодня.   
В 81-ом году Году Дибвиг написал интерпретатор Схемы для Z80 вместе с Джорджем Коном (George Chon). Первая версия интерпретатора поддерживала полноценные продолжения, но в следующем году Дибвиг и Кон отказались от них чтоб больше использовать обычный стек. Как и авторы T, диалекта Схемы, про который мы писали в прошлой части. Использование обычного стека сделало имплементацию намного быстрее первоначальной, но Дибвиг жалел об отказе от полноценных продолжений.   
Дибвиг с Коном попытались написать компилятор, но он был не быстрее интерпретатора, а код занимал больше места.   
В 82-ом году Дибвиг начал имплементацию Curry Scheme, позднее называвшейся C-Scheme.   
Рантайм и интерпретатор первоначально написаны на Паскале на Z80, позднее переписаны на C на VAX. Препроцессор был написан на Схеме, бутстрап с помощью Z80 Scheme.   
В том же 82-ом Дибвиг отправился работать над диссертацией в университет Северной Каролины, нашел работу в местной компьютерной компании Data General. Там он занимался имплементацией Common Lisp. Рантайм он сделал на основе C-Scheme, а компилятор на основе Spice Lisp. Лисперы в коммерческих компаниях постоянно использовали эту имплементацию Common Lisp - общественное достояние и ничего не контрибьютили в нее в свою очередь.   
Поработав над этой имплементацией год, Дибвиг сконцентрировался на работе над диссертацией. Научруком Дибвига был Маго, тот самый разработчик (эмулятора) "древовидной" параллельной ФП-машины, о которой мы писали в прошлой части. Дибвиг разрабатывал для этой машины параллельную Схему осенью 83-го - летом 84-го. Конечно, никакой машины не было, так что Дибвиг писал еще и ее симулятор. На Схеме. Но C-Scheme была слишком медленной для этого. Как и прочие имплементации Scheme. Не T, компилятор которого как раз появился примерно в это же время. T Дибвиг не использовал, может потому, что он только появился. Может из за неполноценных продолжений. Дибвиг попробовал использовать Franz Lisp, но обнаружил, что он до сих пор не поддерживает компиляцию ФВП, только интерпретацию, как в 1960-ом году. И потому не пригоден для функционального программирования. А Дибвиг был из того меньшинства лисперов, которые хотели программировать функционально.   
Так что Дибвиг решил написать полноценный компилятор ФЯ, чтоб писать нужные для диссертации программы. Как Карделли. И это не первое и не последнее решение Дибвига похожее на решение Карделли.   
Дибвиг профилировал свою C-Scheme и решил, что Схемы принято имплементировать неправильно. Авторы схем делают развесистые, разделяющие между собой части замыкания. Которые быстро создавать, но долго потом что-то доставать из них. Так делали имплементаторы Схем со времен Стила и его компилятора Rabbit. Не только они, конечно. Некоторые эмелисты, например авторы CAML, тоже выбрали оптимизацию конструкции замыканий а не доступа к ним.   
От такого подхода имплементатору Схемы отказаться не просто - это важно для быстрых полноценных продолжений. И Дибвиг уже пытался отказаться от полноценных продолжений, но не смог. Но от быстрых продолжений он отказаться смог. Пусть они требуют много копирования, зато все прочие функции будут работать быстрее.   
Так что Дибвиг выбрал плоские замыкания - массивы. Как выбрал и Карделли. В это время Карделли, правда, еще ничего не опубликовал, так что Дибвигу прошлось изобретать плоские замыкания, как и эмелистам, на основе наработок имплементаторов Algol. А именно "дисплеев", описанных в середине 60-х Рэндаллом и Расселом.   
Чтоб использовать плоские замыкания не разделяющие части окружений между собой, Дибвигу пришлось принести еще одну жертву - отказаться от изменения ссылки на месте. Как в LCF-ML и SML, изменяемой может быть только ячейка в куче. Ссылки на нее нужно копировать и потому они должны быть неизменяемыми. И эмелисты и схемеры не собирались их использовать слишком часто, так что с неэффективностью мутабельности можно было смириться. Правда, в ML такие изменяемые ссылки явные, а в коде на Схеме их нужно искать с помощью нелокального анализа, но проблема решаемая.   
Справедливости ради, еще Стил в описании Rabbit отмечал, что планирует двинуться в этом направлении, но так и не двинулся, а просто перестал планировать.   
Как видите, Дибвиг был близок к переизобретению компилятора Карделли исходя из фундаментальных принципов. Но тут в нашу историю снова врывается Карделли, который показал Дибвигу свой инкрементальный компилятор и Дибвиг перешел от переизобретения к заимствованию. Инкрементальный компилятор так понравился Дибвигу, что он решил сделать и свою новую имплементацию Схемы инкрементальным, интерактивным компилятором, а не парой из интерпретатора и компилятора, как принято у лисперов. Так что идейно его компилятор стал ближе к компиляторам VAX ML и Poly и произошедшим от них.    
Тем временем, работа над рантаймом Common Lisp шла без Дибвига не особенно хорошо. Так что летом 84-го его снова наняли в DG поработать над улучшением производительности сборщика мусора.   
Осенью 84-го Дибвиг вернулся к работе над своей Схемой. Взял рантайм C-Scheme, добавил свои наработки для DG Common Lisp. Использовал C-Scheme для бутстрапа компилятора.   
Решение делать компилятор интерактивным и инкрементальным добавило Дибвигу работы - нельзя было использовать системные ассемблер и линкер. Но к концу 84-го года версия 1 Chez Scheme для BSD на VAX-11 была готова и доступна с 85-го.   
Скорость исполнения кода превосходила все "доступные" Дибвигу имплементации Схемы и Лиспа. Правда, в их число не входит компилятор T. И кроме компилятора T и Дибвиговского компилятора Chez Scheme компиляторов Схемы тогда и не было. Похоже, что среди этих компиляторов не было и самых производительных компиляторов LISP.   
Весной 85-го Дибвиг распространил 6 копий своего компилятора, получил положительные отзывы и немедленно основал вместе со своей женой Сьюзан компанию Cadence Research Systems, которая продавала компилятор Дибвига. Этот семейный бизнес был не такими минимальным, как Тернеровский. Дибвиг платил за работу своим (бывшим) студентам. Например Роберту Хибу (Robert Hieb), который писал для компилятора Дибвига бэкенд для MC68K. Но бизнес все же был достаточно минимальным. Дибвиг раздавал лицензии даже не в обмен на машину, на которую портировал свой компилятор, как разработчики Franz LISP / Allegro CL, а в обмен на время на этой машине. Первоначально Дибвиг вообще не имел ни одного компьютера, на котором его компилятор мог бы работать, но позднее накопил на минимальную рабочую станцию SUN. Опять таки, как разработчики Franz LISP / Allegro CL, Дибвиг считал, что разработка на медленной машине положительно повлияла на производительность его компилятора.   
Релиз второй версии состоялся в 1987.   
В третьей версии Дибвиг, Хиб и Карл Брюггеман (Carl Bruggeman) несколько уменьшили цену конструирования продолжений перейдя на сегментированный стек. Это же нововведение позволило обходить ограничения обычного стека на глубину рекурсии. Третья версия вышла в 1989.   
Может быть дело было в том, что поддержка полноценных продолжений не была бесплатной, может потому, что для интерактивного компилятора скорость компиляции может быть важнее, чем скорость исполнения кода, но есть свидетельства того, что эти версии Chez Scheme генерировали не особенно быстрый код. Chez Scheme мог быть, например [Frad91] в 2-5 раз медленнее CAML.   
И по сравнению с новым компилятором Standard ML компилятор Дибвига выглядит еще хуже. Но этот компилятор SML - уже имплементация следующего поколения с очень важным нововведением. Как и Chez Scheme 4, демонстрирующая значительно лучшую производительность. Но появление таких более практичных имплементаций ФЯ уже другая история.   
Десятилетия спустя исходники Chez Scheme открыли и компилятор был использован для имплементации важного ФЯ. Но в 80-е годы коммерциализация компилятора Дибвига, скорее всего, мешала использовать его для имплементации ФЯ. Но такое использование не было полностью исключено.   
Если у языка несколько имплементаций, то вполне возможно использование сочетания хорошей коммерческой имплементации для одних обстоятельств и минимально пригодной для использования свободной - для других. Такое мы в дальнейшем увидим и у тех, кто использовал Common Lisp для имплементации ФЯ.   
Важная имплементация ФЯ, использующая Chez Scheme, появилась не скоро. Но какая-то имплементация все же появилась во времена HOPE-фикации.   
В университете Копенгагена Андерс Бондорф (Anders Bondorf) и Йеспер Йоргенсен (Jesper Jørgensen) занимались частичными вычислениями. А точнее получением компиляторов путем специализации частичного вычислителя для интерпретаторов. Еще точнее - получением таким образом компиляторов ленивых языков из частичного вычислителя для Схемы под названием Similix.   
В Копенгагене начали с минимального ленивого ФЯ LAZY [Bond90]:

```
(map f l = if (= l `())
           `()
           (lazy-cons (f(lazy-car l)) (map f (lazy-cdr l))))  
```

Попробовали компилировать частичным вычислением паттерн матчинг [Jorg90]. Сначала для строгого языка:  
...
Эти наработки использовали для того, чтоб имплементировать еще одну не-Миранду под названием BAWL, что означает "язык (книги) Берда и Вадлера" [Jorg92]. И язык в книге Берда и Вадлера не особенно отличается от Миранды.   
Имплементация Схемы, которую в Копенгагене используют с авто-проектором Similix - это Chez Scheme 3.2. Но Similix можно было использовать и с T3.1 [Kant93]. Но измерения производительности, которые мы нашли, были только для Chez Scheme.   
Получаемый частичным вычислением компилятор генерировал код, работающий в 55-69 раз быстрее наивного интерпретатора используемого для специализации. Но Миранда все же не совсем наивный интерпретатор и ускорение по сравнению с Миранда 2 было всего в 1.29 раз для LAZY и в 1.4-4.6 раз в случае BAWL. Так что BAWL был, вероятно, самой медленной быстрой Мирандой.   

### Частичная орбита

В предыдущей части мы оставили разработчиков и имплементаторов T весной 84-го на версии 2.8. Этой весной (пока что) основной имплементатор T Джонатан Рис уже работал над новым компилятором. Но эта работа набрала обороты только летом, когда руководитель проекта Джон О'Доннел договорился со своим знакомым из DEC, чтоб йельские разработчики T поработали там над имплементацией Лиспа для нового RISC-процессора DEC Titan. Этим знакомым был Форест Баскетт, который уже поучаствовал в нашей истории как руководитель разработки рабочей станции SUN.   
И летом 84-го семеро из Йеля поработали в DEC [Shiv2001]. Или шестеро [Rees2010], зависит от источника. Но поработали они не над тем, что ожидал Баскетт. Он ожидал, что они сделают версию T для Titan. Вместо этого, они стали писать новую версию T3 и новый компилятор T - Orbit.   
Джонатан Рис руководил этими работами и писал линкер. Дэвид Кранц, который уже поучаствовал в нашей истории как автор компилятора ALFL, писал генератор кода. Ричард Келси (Richard Kelsey) писал трансформирующий оптимизатор. Норман Адамс (Norman Adams) писал бэкенд, Джим Филбин (Jim Philbin) - рантайм и генератор кода. Олин Шиверс (Olin Shivers) и более известный другими своими работами Джон Лампинг (John Lamping) писали DFA (Data-flow analysis). Шиверс утверждает, что до того никто DFA для компилятора Лиспа не делал. Не вышло и в этот раз, так что ни одна строка, написанная Шиверсом в DEC, в T3 так и не попала.  
 Такая команда уже больше похожа на команду работающую над продвинутым коммерческим Лиспом, а не на команду, пишущую компилятор Эдинбургского языка или Схемы. И они поставили перед собой очень амбициозную цель написать компилятор Схемы, который мог бы по скорости генерируемого кода посоревноваться с мейнстримным компилятором вроде компилятора C.   
Герои этой истории до сих пор часто цитировали работу Стила о его компиляторе Rabbit, использующем CPS-преобразование. Но компиляторы которые они писали были больше похожи скорее на первый компилятор Пролога Уоррена, а не на компилятор Стила. Даже сам Стил, написавший компилятор S-1 NIL, основу компилятора T2, не стал повторять свой Rabbit.   
Но Рис решил, что теперь-то, в 84-ом году пришло время повторить Rabbit, и даже масштабировать proof of concept Стила в серьезный, практичный компилятор.   
И это удалось сделать. Конечно, не сразу. Не достаточно быстро для того, чтоб Баскетт получил ожидаемый T для Titan к концу лета. Не первый Титан для которого не успели сделать амбициозный компилятор в этой истории.   
Возможно, вспоминает Рис, Баскетт был бы намного счастливее, если бы они действительно сделали то, что обещали ему. А именно - компилятор Схемы для новой машины DEC. Хотя-бы в первом приближении. Может быть, предполагает Рис, Баскетт пригласил бы их поработать еще. Но они не сделали что обещали, Баскетт был не особенно счастлив и не пригласил.   
Работы над Orbit продолжились в Йеле. Но не слишком интенсивно. Первый руководитель проекта Джон О'Доннел ушел и после его ухода новым руководителем стал более известный другими своими проектами Пол Худак.   
Осенью 84-го ушел из проекта и Рис. Чтоб заняться своей диссертацией не имеющей отношения к имплементации Схем. Вместо него основным мейнтейнером T стал Дэвид Кранц. Тот самый, разрабатывавший под руководством Худака временно комбинаторный компилятор ALFL.   
Худак в рассылке [TMail] объяснял, что никто никогда не обещал закончить T3 Orbit к концу лета или к концу 84-го года. Но это нужно было объяснять. Не только Форест Баскетт думал, что ему что-то там обещали.   
В 85-ом году Orbit, по видимому, как-то заработал. В 86-ом вышла статья о нем [Kran86]. Худак стал научруком у Келси и Кранца. Диссертация Келси была готова в 86-ом, а диссертация Кранца - в 88-ом [Kran88].   
Релиз новой версии T 3.0 с компилятором Orbit состоялся только в январе 87-го [Phil88].   
Удалось ли сделать компилятор ФЯ, генерирующий быстрый код? Его создатели считают, что да. Шиверс утверждает, что Orbit был самым производительным компилятором Схемы на протяжении десяти лет. Кранц в своей диссертации приводит бенчмарки, в которых Orbit немного лучше чем Pascal на Apollo. Это может звучать не очень впечатляюще, но учтите, что это системный компилятор, котором в Apollo собирали ОС для своих машин. Orbit-код также быстрее, чем выхлоп Lucid Common LISP на Sun 3, часто в разы быстрее.   
Естественно, новая версия компилятора T была использована как основа имплементации функционального языка Худака ALFL.   
В очередном описании [Blos89] компилятора ALFL от ноября 88-го года Худак, Джонатан Янг (Jonathan Young) и Эдриенн Блосс (Adrienne Bloss) рассказывают о своих переменных успехах. Не перечислены среди авторов статьи, но работали над компилятором: Фред Дуглас (Fred Douglas), Йен Тейлор (Ian Taylor) и Мария Гусман (Maria Guzman).   
Компилятор, который они распространяют в это время - Xalfl 3.0. Более новая, продвинутая версия, с которой пока экспериментируют только в Йеле называется Nalfl.    
Авторы компилятора ALFL считают, что проблема создания компилятора строгого ФЯ, который генерирует код с той же производительностью, то у мейнстримных языков - решена. Решение - это Orbit. Вот с трансляцией ленивого ФЯ в такой строгий ФЯ пока не все хорошо. Но работа идет. Авторы работают над анализом строгости и завершимости, частичным вычислением. Все это более-менее работает. Ищут структуры, на которые есть только одна ссылка и которые можно изменять на месте. Это все пока более экспериментально.   
Авторы разочаровались в своем необычном подходе к оптимизации путем трансляции в комбинаторы, частичной их редукции и обратной трансляции в лямбды. Занимает слишком много времени и генерирует слишком много кода. Теперь они работали над более традиционным частичным вычислителем.   
ALFL транслируется в код на T в котором значения, оставшиеся ленивыми после анализа строгости, представлены специальными конструкторами-обертками. Вроде тех, которые программисту, пишущему на Ocaml нужно использовать явно, если хочется опциональной ленивости. Это не самый быстрый способ имплементации ленивых значений, особенно, в случае T, рантайм которого не поддерживает такие обертки. Так что авторы компилятора ALFL особенно заинтересованы избавиться от ленивости везде, где это возможно. Так же как авторы LML и в отличие от авторов Ponder, авторы ALFL считают, что полной ленивостью можно и пожертвовать ради лучшей производительности.   
Тем не менее, даже в случае, когда от всей ленивости избавиться можно, например вычисляя числа Фибоначчи, Nalfl может отставать от T в 2.8 раз. В более сложных случаях ALFL может быть медленнее T в 8 раз, а может и в 40 раз. Справедливости ради, в паре бенчмарков разница небольшая. До сих пор результаты имплементаций 70-х (и Миранды) и компиляторов 80-х группировались в два хорошо разделяемых облака точек. Но ALFL мог легко демонстрировать и результаты как в 70-е и как в 80-е, в зависимости от бенчмарка. Но разработчики компилятора не унывали. Писали, что впереди еще много работы.   
ALFL устарел. Его авторы считали, что отсутствие в нем типов - серьезная проблема для компиляции в быстрый код. И Худак, в месте с другими героями этой истории, уже работали над заменой. Хотя реально существующий компилятор все еще компилировал ALFL, но в статье о нем примеры кода были уже на другом языке. Более современном и HOPE-фицированном. Компилятор ленивого ФЯ на основе Orbit пока еще не имплементировал этот новый язык. Но Худак планировал, что его компилятор станет первым компилятором гораздо более известной и важной не-Миранды, чем BAWL.   

### Карри

> Вы знаете, Хаскелю на самом деле никогда не нравилось имя Хаскель.   
> Вирджиния Карри   

Итак, в 87-ом году многие герои нашей истории хотели писать на Миранде и имплементировать Миранду. И если это по ряду причин было невозможно - имплементировать что-то не слишком сильно от Миранды отличающееся.   
Но что если вместо множества не-Миранд сделать единую не-Миранду, в которую каждый имплементатор не-Миранд добавит то, что ему нужно, если это не сильно помешает остальным? Даже сам Тернер продвигал Миранду как некий единый ленивый HOPE, но эта цель, конечно, плохо совмещалась с другой целью Тернера - продавать интерпретатор.   
Очередная конференция FPCA в 87-ом году проходила 14-16 сентября в Портленде, Орегон. И по пути на эту конференцию Пейтон-Джонс остановился в Йеле, чтоб пообщаться с Худаком. И Худак с Пейтон-Джонсом решили во время конференции организовать встречу функциональных программистов, которым интересно поучаствовать в проектировании еще одного единого ФЯ. Для тех, кому не подошел другой единый ФЯ - Standard ML. Первым поддержавшим идею был Вадлер, который тоже заехал к Худаку по пути на конференцию. Кто были следующие? Точно не известно.   
Самый ранний дошедший до нас список соавторов нового единого языка от 11 мая 88-го года помимо Худака, Пейтон-Джонса и Вадлера включает и многие уже знакомые нам имена. Ричард Берд,  работавший с Вадлером над Orwell в Оксфорде. Автор Id Арвинд, представлявший МТИ. Изобретатель суперкомбинаторов Джон Хьюз, представлявший вместе с Вадлером университет Глазго. Разработчик и имплементатор языка Ponder Джон Фейрберн из Кембриджа. Автор и имплементатор Lazy ML Томас Йонссон из Технологического Университета Чалмерса. Один из авторов CAML Ги Кузино из INRIA. Кузино второй, помимо Вадлера, но не последний поучаствовавший в создании и SML и Haskell, хотя больше известен другими своими работами. Один из авторов CLEAN Маринус Плазмейер. Моделировавший в Орегоне аппаратную G-машину Ричард Кибурц. Разработчик спецмашины ALICE Майкл Рив, работающий вместе с Дарлингтоном в Имперском Колледже Лондона. И даже один из изобретателей ленивости Дэвид Уайз, представлявший университет Индианы. Уже упоминался как организатор конференции, но пока не сыграл значительной роли Джозеф Фазель (Joseph Fasel) из Лос-Аламосской национальной лаборатории.   
Некоторые функциональные языки представляют больше одного человека, но не LML. Ни Августссон ни Йонссон не хотели заседать в комитете, но считали, что разработчик LML должен поучаствовать, тянули жребий, Йонссон проиграл и стал членом комитета [Augu25].
Это не все те, кто приложил руку к черновику описания языка. В описаниях вводится даже больше категорий поучаствовавших в создании языка, чем в описаниях SML. Но самый ранний из этих списков только от декабря 88-го. Помимо редакторов и авторов отмечают тех, кто принял заметное участие, это Джон Ланчбери (John Launchbury) и Стефен Блотт (Stephen Blott) из Университета Глазго и Раман Сундареш (Raman Sundaresh) и Мария Гусман (Maria Guzman) из Йеля.   
Также отмечают комментарии и предложения от целого ряда исследователей, многие из которых нам уже знакомы. От работавшего вместе с Фейрберном над Ponder Стюарта Рея. Комментировал и предлагал бывший коллега Тернера по Сент-Эндрюсу Тони Дейви (Tony Davie). Автор первой имплементации SASL и один из авторов S-algol. Занимавшийся чисто функциональными операционными системами Саймон Бернард Джонс (Simon Bernard Jones). Коллеги Худака по Йелю Ку Cьяучьенг (Siau-Cheng Khoo), Амир Кишон (Amir Kishon), Рик Мор (Rick Mohr) и разработчик компилятора ALFL Лорен Смит (Lauren Smith). Коллега Фазеля по Лос-Аламосской национальной лаборатории Рэнди Михелсен (Randy Michelsen). Артур Норман (Arthur Norman) - преподаватель Пейтон-Джонса, познакомивший его с функциональным программированием. Работавший вместе с Пейтон-Джонсом над FLIC Майкл Джой (Mike Joy) из Университета Уорика. Джон Робсон (John Robson), один из улучшателей G-машины, соавтор Пейтон-Джонса. Колин Ренсимен (Colin Runciman) из Йорка, разработчик экспериментальных приложений для спецмашины Пейтон-Джонса. Пол Отто (Paul Otto) из Университетского Колледжа Лондона и рассылки языка T.   
Ларне Пековски (Larne Pekowsky) и Стеф Джоостен (Stef Joosten), про которых мы ничего не знаем.   
Джон Кивли (John Kewley) вспоминает [Kewl06], что был в группе, которая обсуждала дизайн единого ленивого ФЯ, когда он, по видимому, еще не имел названия, то есть между сентябрем 87-го и январем 88-го. В группе состояли: разработчик NPL и "имперского" HOPE Дарлингтон и его коллеги по работе над HOPE Фил Бротон (Phil Broughton), Том Томпсон (Tom Thomson) и сам Джон Кивли, коллега Пейтон-Джонса по Университетскому Колледжу Лондона Крис Клак (Chris Clack), разработчик FAST Хью Глейзер (Hugh Glaser), участники проекта Flagship Пол Уотсон (Paul Watson) из университета Ньюкасла, и Ник Холт (Nic Holt) из ICL. Кивли считает, что какой-то вклад в разработку участники группы сделали. Но от всего этого процесса осталось не особенно много следов. Разве что только благодарности за комментарии и предложения для Клака, Томсона и Холта.   
Не упомянутый Кивли, но явно связанный с этой группой Майкл Рив среди авторов описания, но Пейтон-Джонс писал [SPJ91b], что особо активного участия Рив не принимал.   
На конференции читали доклады, авторами которых были комитетчики Фейрберн, Пейтон-Джонс, Уайз, Йонссон, Худак, Вадлер, Хьюз и Ринус Плазмейер. Из тех, что получили благодарности за предложения и комментарии или поучаствовал в группе про которую вспоминает Кивли - Стюарт Рей, Пол Уотсон, Дарлингтон, Крис Клак и Колин Ренсимен. 
Но также и те, кто не стал соавтором описания, но, в принципе мог и поучаствовать в первой встрече будущего комитета. Например, один из изобретателей dataflow и, вместе с МакКвином, паттерн-матчинга - Жиль Кан, сам МакКвин, эмелисты Аппель и Реппи, разработчик SKIM Кларк, остальные авторы CLEAN.   
Понятно, что не все перечисленные участвовали с самого начала, но, надеемся, что практически все участвовавшие с самого начала так или иначе упомянуты.   
Вторая встреча части комитета состоялась в декабре 87-го в Университетском Колледже Лондона. Но значительная часть работы проходила без очных встреч.   
После начала разработки Standard ML прошло только четыре года, но новый способ разработки языков - с помощью обсуждения в интернете - стал уже доступен и не таким богатым как лисперы разработчикам языков. Комитет, назвавшийся "FPLang Committee", не ограничился периодическими встречами, но завел закрытую рассылку fplangc@cs.ucl.ac.uk на сервере Университетского Колледжа Лондона, в котором пока еще работал Пейтон-Джонс.   
На первом заседании комитета во время FPCA '87 не было принято много решений. Но, по крайней мере решили, что за основу будущего единого ленивого ФЯ нужно взять уже существующий ленивый язык. Этим существующим языком была, конечно, Миранда.
Тернер не участвовал в первой встрече комитета, так что не мог отказаться сразу, но отказался, как только ему представили такую возможность. Участники этой беседы со стороны комитета описывают беседу с Тернером как "короткую". Но, правда, "сердечную".
Тернеру по видимому не понравилась идея, что будущую Miranda++ можно будет имплементировать и распространять всем желающим. Он попросил, чтоб единый ленивый ФЯ существенно отличался от Миранды. Но отказался от участия в комитете, в котором он мог бы проследить за тем, чтоб отличие действительно было бы существенным.   
Так что второй значительный проект по созданию единого ФЯ очень быстро свернул с пути первого проекта. У этого проекта не оказалось своего Милнера.   
На работу комитета в 88-89 годах Вадлер и Хьюз получил от SERC 2.5 тыс. фунтов ($9600 в 2026) [Wadler], но основные расходы на "стандартный ленивый функциональный язык" были для SERC еще впереди.   
Следующее заседание комитета состоялось 9-12 января 88-го года в Йеле. Комитетчики решили, что новый язык должен подходить для преподавания, исследовательской работы и для написания больших, серьезных программ. У языка должна быть описана формальная семантика. Любой желающий может имплементировать язык. Язык должен быть основан на общепризнанных идеях. Эксперименты должны быть в исследовательских расширениях, а не в базе языка. Как и в случае с SML, считали что эксперименты с ядром языка уже закончились. Но, конечно, на самом деле они только начались.   
Базой языка должен стать OL. Та версия Orwell, которая еще не была окончательно Мирандизирована ради книги Берда и Вадлера. Но, прямо скажем, от Miranda и она отличалась не особенно. Так что комитетчики может и следовали букве соглашения с Тернером, но следовали ли они духу этого соглашения? Этот вопрос не такой и важный. Не только потому, что Тернер все равно не мог ничего сделать.  
Что комитетчики решили сделать обязательно до конца второго заседания - это выбрать название. Новый единый ФЯ остался не только без своего "Милнера", не получил язык и "временное" название, ставшее постоянным, которое получили Standard ML и Common LISP присоединив что-то к имени базового языка. Некоторые комитетчики, правда, все равно предложили названия построенные по этому принципу, но без очевидного языка предшественника победить такому ленивому названию было сложнее.   
Процесс выбора имени предложил Вадлер. Сначала комитетчики предлагали имена и записывали их на доске. Имена были такими: Semla, Haskell, Vivaldi, Mozart, CFL (Common Functional Language), Funl 88, Semlor, Candle (Common Applicative Notation for Denoting Lambda Expressions), Fun, David, Nice, Light, ML Nouveau, Miranda Nouveau, LML Nouveau, Mirabelle, Concord, LL, Slim, Meet, Leval, Curry, Frege, Peano, Ease, Portland, и Haskell B Curry.  
Теперь каждый комитетчик мог вычеркнуть название, которое не нравится больше всех остальных. В результате осталось название Curry. 
На следующий день, испугавшись ассоциаций с Тимом Карри, комитетчики переименовали язык в Haskell. О языке, который называется Curry и сегодня - в следующей главе.   
Комитет не смог следовать ими самими же определенным правилам выбора названия. И это событие было бы полезным учесть, чтоб определить дальнейшие ожидания.   

### ML Nouveau

> Хаскель напоминает подчищенную версию Lazy ML, подправленную так, чтобы быть немного более похожей на Miranda, что может ввести в заблуждение невнимательных.   
> Дункан Синклер   

Известно, что комитет собирался разрабатывать Хаскель на основе Миранды, а после отказа Тернера на основе Orwell. Что, по большему счету, одно и то же. Но так вышло, что не смотря на такие планы комитета, Хаскель сначала получился языком "существенно отличающимся" от Миранды. В том числе и по мнению самого Тернера. Речь тут не идет о том, что в Хаскеле появилось много чего из того, чего в Миранде не было. Это само собой разумеется. Дело в том, что в Миранде было, а в Хаскеле, по крайней мере первоначально, не было. Не было в Хаскеле 80-х. Хаскель, со временем, стал походить на Миранду гораздо больше.   
К сожалению, процесс создания не похожего на Миранду Хаскеля 80-х на основе похожего на Миранду OL документирован не так хорошо, как, например, процесс создания Standard ML. Да, проектирование Haskell как и Common Lisp обсуждали по электронной почте. Но создатели Хаскеля решили не публиковать архивы этих обсуждений, хоть они и сохранились [Fair06]. По какой-то причине пишущие историю Хаскеля [Huda07] не выложили массу оцифрованных материалов которые использовали, как сделали пишущие историю SML. И по какой-то причине постарались анонимизировать большинство решений.   
К счастью, в 90-е обсуждение переместилось в открытую рассылку, архив которой доступен для всех желающих. И это означает не только то, что трансформация не похожего на Миранду Хаскеля в Хаскель, на Миранду похожий, документирована гораздо лучше. Участники закрытых обсуждений 80-х обсуждают эти обсуждения. На этот раз на открытом форуме.   

#### Haskell 0

Язык Вадлера и Берда OL/Orwell, который планировали сделать основой Хаскеля был вариацией популярного языка, но языка довольно необычного. Этот язык был последним продуктом многолетней работы Тернера по созданию языка уравнений более или менее очищенного от таких конструкций из языков выражений, для которых не было альтернатив в языках уравнений. Например, таких типичных для функциональных языков конструкций как лямбда и `let in`. И этот специальный интерес Тернера не разделяло большинство авторов функциональных языков.   
Фейрберн и Рей не довели HOPE-фикацию своего языка Ponder даже до добавления паттерн-матчинга, это был типичный язык выражений. Остальные продвинулись дальше. ALFL Худака продвинулся так же далеко, как SASL и KRC, не потеряв при этом никаких конструкций с которыми боролся Тернер. И Ponder и ALFL, по видимому, были заброшены с началом разработки Хаскеля, на который их авторы и переключились. Работы над HOPE-фикацией Id продолжались Арвиндом и Нихилом одновременно с созданием Хаскеля и группа Арвинда в МТИ перешла с Id на Haskell не сразу, только в 90-е годы. В Id добавляли конструкции обоих разновидностей и не стремились к минимализму. Авторы Lazy ML Йонссон и Августссон предполагали, что Haskell заменит LML, но Августссон продолжал добавлять новые фичи в LML пока Хаскель не был готов хотя-бы в первом приближении. Lazy ML прошел полную HOPE-фикацию раньше многих других и его авторы тоже не боялись сочетать конструкции делающие одно и то же разными способами. Даже Вадлер, сделав похожий на Миранду язык приветствовал такое сочетание, но об этом позже. Разработчики языка Advanced Intermediate - другой не-Миранды, которые поучаствовали в создании и имплементации Хаскеля тоже не боролись с `let`. Но не все соавторы Хаскеля переключились на него. Посмотрим на тех авторов, которые продолжили развивать свои языки. Плазмейер и прочие авторы CLEAN начали с минимального языка похожего на тернеровский, но по мере добавления фич он стал больше похож на вышеперечисленные языки. Кузино работал над не менее необычным языком, чем тернеровские. Но в CAML избавлялись от конструкций из языков уравнений, которые дублировали конструкции языков выражений. CAML был анти-Мирандой. Ну и разработчики HOPE и Standard ML создали языки совмещающие элементы из языков уравнений и языков выражений первыми. Сомнительно, что их вклад в разработку Хаскеля способствовал его сходству с Мирандой.   
Многим из перечисленных выше нравились языки Тернера, но когда они предлагали использовать какую-нибудь идею Тернера, они обычно не имели в виду идею об исключении лямбд из функциональных языков. Среди участников проекта по созданию свободной Миранды было недостаточно желающих делать Миранду. Ну или Orwell, после того, как не удалось получить разрешение делать Миранду.   
Что само по себе не проблема, не мешает сделать Миранду со всеми любимыми расширениями участников разработки. Со временем так и произошло, это в основном описывает получившийся результат. Но путь к этому результату не был прямым.
В первых же набросках Вадлера Haskell стал более обычным ФЯ, чем Miranda и Orwell. К Orwell-основе он добавил выражение `where`, а `where`-часть декларации Тернера переименовал в `with`. В языке были какие-то гарды, но сложно сказать как именно выглядящие. Представление об этой версии Хаскеля можно составить только из обсуждений на открытом форуме годы спустя и мы не видели никаких примеров кода на этом языке.   
Получается что-то уже похожее на современный Хаскель. Нужно сделать один шаг - заменить выражение `where` на выражение `let` и оставить ключевое слово `where` для конструкции с областью видимости распространяющейся на гарды. Но, как и можно ожидать от языка, спроектированного комитетом, равнодействующая всех усилий дернула язык в неожиданном направлении.   
В отличие от почти всех перечисленных выше языков разработкой Хаскеля занимался комитет. И судя по более поздним обсуждениям в открытой рассылке это очень беспокоило комитетчиков. Если помните, это беспокоило и разработчиков SML. Милнер даже написал, что он с коллегами по комитету считает, что хороший язык комитет не разработает. Милнер решил эту проблему просто: провозгласил, что SML разрабатывает не комитет. Решение, которое нашли разработчики Хаскеля имело более разрушительные последствия.   
Язык в котором смешаны и конструкции языков уравнений и конструкции языков выражений выглядит как два языка в одном, пишут они в открытой рассылке. Выглядит подозрительно, как язык, который спроектировал комитет. То, что языки HOPE, ALFL, LML, Id, Clean выглядят так, хотя разработаны не комитетом, их не смущало. Видимо, только язык, который разрабатывает комитет не может позволить себе выглядеть как язык, который разрабатывает комитет.   
Чтоб избавиться от этого нежелательного вида не впадая в пуризм Миранды или CAML нужно просто выбрать из каждой пары аналогичных фич самую лучшую. Язык все равно будет смешением фич из языков уравнений и языков выражений, но все в нем можно будет делать одним способом. Не всегда понятно насколько фичи взаимозаменяемы и насколько их сочетание избыточно и решения комитета об этом регулярно менялись.   
Своему решению о том, что основой Haskell будет OL/Orwell комитет следовать тоже не смог.   

#### Pre-Haskell

В начале апреля 88-го состоялась следующая очная встреча комитета в Глазго. Про эту встречу пишут, что на ней были решены многие ключевые вопросы, но не пишут какие. На этой же встрече решили, что редакторами описания будут Худак и Вадлер [Huda07].   
В начале мая вышел отчет Йельского университета [Huda88a] содержащий примеры на предварительной версии Хаскеля и его короткое описание. Худак пишет, что пока что решались важные вопросы семантики, а вопросы конкретного синтаксиса пока не решены. Так что синтаксис может еще существенно поменяться. Так и вышло. Но Худак, по видимому, не считал, что обсуждение синтаксиса потребует много времени, потому что предполагал, что первый черновик описания Хаскеля будет готов 1-го июня того же 88-го года, а более-менее готовая версия - в августе. Этого не произошло.   
В результате нового, комитетобоязненного подхода к дизайну, следующий шаг привел не к современному Хаскелю, а к тому, что называлось pre-Haskell.   
В pre-Haskell были уравнения с ПМ, но не было гард. Вместо них выбрали условное выражение - тернарный оператор, как в CPL, ISWIM, LCF/ML: `p -> x , y`. Между выражением `where` и  `where` Тернера неожиданно выбрали выражение. Другими словами пре-Хаскель был типизированным SASL с лямбдами как в LCF/ML `\x y.e`. И list comprehensions как в KRC `[(x,y) | x<-xs; y<-ys]`. Такой Хаскель мог бы появиться в 79-ом году, но появился в 88-ом.   
Так что наш обычный пример выглядел так:

```haskell
map f []     = []
map f (x:xs) = f x : map f xs

map (\x. x + y) [1..3] where y = 1
```

И `filter` выглядел так:
...
В июле 88-го в Глазго состоялась первая встреча рабочей группы 2.8 IFIP по функциональному программированию. В этой группе состояли многие герои этой истории и Хаскель-комитет провел заодно заседание совмещенное с этой встречей и следующими.    
Это была последняя встреча комитета перед выпуском двух черновиков описания. Они вышли один за другим TR656 в октябре 88-го, а TR666 в декабре того же года. Октябрьский отчет до нас не дошел, но декабрьский дошел, спасибо Филипу Вадлеру!   
Между этими отчетами в ноябре была написана статья про компилятор ALFL [Blos89], про которую мы уже писали выше. ALFL авторам уже поднадоел, так что примеры в ней на Хаскеле, но они не позволяют определить что поменялось между отчетами. Хаскель там не содержит ничего отличающегося от описанного в декабрьском отчете.   
В декабрьском отчете новый список комитета и в нем больше нет Берда, Кузино и Плазмейера. Упоминания о них не исчезли полностью, но они перенесены в список благодарностей за комментарии и предложения.   
Это не обязательно означает, что на встрече комитета в июле случилось что-то интересное. Это вероятно оставило бы после себя больше следов и имена из списка членов комитета переходили в список благодарностей регулярно. Но интересно, что в этом случае комитет потерял представителей разработчиков довольно важных для нашей истории языков - Clean и CAML.   
Это, конечно, напоминает о похожем событии в истории Standard ML. Возникает вопрос, был ли у второго проекта по созданию единого ФЯ свой "CAML"? Не только проект по созданию Хаскеля был покинут Кузино как незадолго до того проект создания Standard ML. Это событие, по видимому, снова совпало с решением ограничить язык только линейными паттернами. Почему Кузино вообще был в Хаскельном комитете? Не забывайте, что в это время существовала и ленивая версия CAML.    
Не смотря на это, на роль аналогичную той, которую сыграл CAML в истории SML, больше подходит CLEAN. По крайней мере, это альтернативный ленивый язык, который существует в каком-то смысле и сегодня. Разумеется, CAML был гораздо более успешным восстанием против SML комитета.   
 Мы подозреваем, что причины покинуть комитет для Плазмейера были больше похоже на причины Тернера не участвовать в нем с самого начала. И перспективы ленивого CAML были не особенно радужными. Но большая часть истории Clean и CAML еще впереди, будем надеяться, что ответ еще будет найден.   
Эти места в комитете заняли двое уже поучаствовавших в нашей истории. Разрабатывавший язык Id вместе с Арвиндом Нихил. Один из разработчиков ALFL Джонатан Янг, коллега Худака по Йелю. И еще один новый герой - Брайан Боутел (Brian Boutel) из Университета Виктории, Новая Зеландия. Боутел разрабатывал свой собственный функциональный язык Tui, но главный его вклад еще впереди.   
По видимому, комитет пытался перераспределить наилучшим образом знаки препинания между пользовательскими операторами и синтаксическими конструкциями, в основном тернарным оператором и лямбдой.  
Новый условный оператор `p => a ; b`, правда, только опционально тернарный. Можно писать и просто `p => a`. Если условие не выполняется - программа завершается с ошибкой. Есть еще вариант - "верифицирующее выражение" `x = y` означающее `(x == y) => x ; bottom`. В Хаскеле пока нет средств обработки ошибок, но много средств их создания.   
В Хаскеле уже есть не только синтаксис на отступах, но и C-подобный синтаксис блоков с фигурными скобками и точками с запятой как в ALFL. Но одинарная точка с запятой уже занята условным выражением, так что нужно использовать двойную.   
Новое условное выражение, в свою очередь, новое с `=>` потому, что `->` занято новой лямбдой.   
Новая лямбда хуже чем в пре-Хаскеле. Она не поддерживает облегченный синтаксис для каррированной лямбды, нужно строить стрелку за стрелкой: `x->y->(x-y)` вместо `\x y . x-y`. Отсутствие поддержки для карринга, впрочем, не означает, что есть поддержка для многоветочного ПМ как в лямбдах в HOPE и SML. В этом Хаскеле лямбда просто хуже любой из двух основных разновидностей. Как в LML, но с другим синтаксисом. Но в этом Хаскеле хотя-бы есть сечения современного вида. И лямбда - не единственное, что в этой версии как в LML.   
Обсуждался не только синтаксис лямбд, рассматривали и добавление в Хаскель лямбд с многоветочным ПМ. Через десятилетия это действительно произошло, но не в этот раз. От этой неудачной попытки в лямбдах остались следы - гарды.   
В самом первом варианте Хаскеля были гарды, вероятнее всего как в Miranda и Orwell. Были ли гарды в вариантах между первым и декабрьским TR666? Трудно сказать. В этот период гарды не используются в коде стандартной библиотеки и даже в примерах в декабрьском черновике, разве что только в примерах в описании гард в этом черновике.   
Может потому, что они появились в последний момент перед завершением этого черновика. Может быть они считались временными или постоянно менялись. Но и условные выражения несколько раз изменились.   
Может их просто не любили из-за плохого взаимодействия с выражением `where`. Автор кода стандартной библиотеки и не только он жалуются на то, что нет тернеровского `where`, который и был изобретен ради хорошего взаимодействия с гардами.   
Но в декабрьском отчете гарды уже точно есть. Но это не гарды современного вида. Это гарды примерно как в LML, но синтаксис отличается. В LML гарды выглядели так:
...
Если сравнить их с гардами в KRC, Orwell, Miranda, то видно два основных отличия. Во первых, условие находится ближе к паттерну, слева от `=`, а не справа, после выражения-результата. Также как и в современных гардах. Но чем они отличаются от современных - нужно продолжать писать имя и параметры функции для всех ветвей.   
И в Haskell 88.12 гарды похожи
...
Но, понятное дело, повторять это имя и параметры хочется не всем. Так что из языка Худака ALFL позаимствовали `'` означающую "имя и параметры как в строке выше"
...
отсутствие условия - это то же, что и `True`.   
Один такой гард можно приписать и лямбде: `x {x>0} -> x` ветвь у лямбды осталась одна после безуспешной попытки разрешить много, так что это еще один способ завершить программу.   
Гарды сделали не справа от результата функции как в Миранде, а слева как в LML потому, что в "плоском" языке уравнений таком как KRC или Miranda эти результаты обычно небольшие. Но в Хаскеле много средств для построения больших выражений и если эти большие выражения строятся между паттерном и гардом, гарды уезжают куда-то далеко и такой код читать сложно [Huda90a].    
В Orwell порядок уравнений с ПМ не имеет значения, как в HOPE. Такое обсуждали и для Хаскеля. Но определить полноту перекрытия в языке с произвольными гардами нельзя, так что от них отказались. В этой версии Хаскеля порядок уравнений значение имеет, как в языках Тернера, Прологе, SML, ну и в современном Хаскеле [Huda89].   
Паттерны уже линейные, тоже как в современном Хаскеле, а не как в Миранде. Нелинейные паттерны не попали в Хаскель, по видимому, из-за того, что не очевидно что вычисляется, а что за ненадобностью уже нет как в таком паттерне `[x,x,x]`. Было ли это серьезной проблемой в то время, когда думали не сделать ли порядок паттернов не значимым как в Orwell?   
Паттерны вроде `x` или `(x,(y,z))` и прочих единственных конструкторов пока что irrefutable как в Miranda.   
Но, в отличие от Миранды, в этом Хаскеле уже есть `as` паттерны, но пока что вида `x=pat` вместо современного `x@pat`.   
Есть `n+k` паттерны и `case of` практически современного вида, если не считать гард.   
List comprehensions отличаются отсутствием локальных определений в `let` и синтаксически: вместо `|` разделителя используется `||`.   
`[]` является сахаром для `Nil` в выражениях, а `[a]` для `List a` в типах. Но не в определениях АлгТД, так что наш обычный пример выглядит так:
...
или так, если не захотите использовать отступы:
...
Может быть вы не захотите и ставить `;;` в начале строк, и можете их там не ставить, но в примерах они стоят именно там.   
один из АлгТД в примерах записан не так как прочие:

```
type Bool ::= False | True  
```

Возможно, что это неподчищенные следы предыдущего способа определять АлгТД.   

#### Haskell 89.05

Следующая встреча комитета состоялась в первых числах мая 89-го на курорте Мистик, Коннектикут недалеко от Йельского университета. Судя по более поздним комментариям, на этой встрече еще обсуждали не сделать ли `let in` вместо выражения `where` и один из авторов LML Йонссон защищал `where` [Fase91].   
О состоянии Хаскеля в мае 89-го мы знаем, в основном, из статьи Худака по истории функционального программирования [Huda89] с примерами на Хаскеле.   
В этом Хаскеле лямбды уже выглядят знакомо. Тернарный оператор уже заменили на привычный `if then else`. Исчезли двойные разделители `||` в list comprehensions и `;;` в альтернативном синтаксисе для тех, кто не любит отступы.   
Но гарды выглядят знакомыми только на первый взгляд. Разделитель между паттернами и градом уже современный:   
...
Но имя функции все еще нужно повторять. Ну или использовать фичу из ALFL:
...
По видимому обсуждались и вложенные гарды внутри паттернов `(pat | pred)` но решили отложить их на будущее, для Haskell 2 [Huda90a].   

```haskell
filter p []             = []
'      p ((x | p x):xs) = x : filter p xs
'                       =     filter p xs
```

Главное отличие в синтаксисе ядра языка от современного Хаскеля - выражение `where` - пока осталось.   

#### Haskell 1.0

Первая версия описания Хаскеля вышла 1-го апреля 1990 [Huda90]. Ровно через пять лет после первого описания предыдущего языка Вадлера Orwell, которое тоже вышло 1-го апреля.   
В этот раз комитет только увеличился, к нему присоединился Кевин Хаммонд, герой предыдущей главы, тот, что имплементировал чисто функциональный ленивый "Standard" ML.   
В секции благодарностей за отзывы и предложения отмечены как старые знакомые, так и новые люди. Один из авторов и имплементаторов Lazy ML Леннарт Августссон. Который хотя и выиграл у Йонссона право не участвовать в работе комитета, но не выдержал и поучаствовал таки. Ричард Келси (Richard Kelsey), который писал трансформирующий оптимизатор нового компилятора T Orbit. Еще коллеги Худака по Йелю Прадип Варма (Pradeep Varma), Джон Петерсон (John Peterson) и Дюк Бриско (Duke Briscoe). Коллеги Фазеля по Лос-Аламосу Бонни Янтис (Bonnie Yantis), Боб Хиромото (Bob Hirohito) и Олаф Любек (Olaf Lubeck). По видимому, жена Фазеля, Патриция (Patricia Kroll Fasel), как и он исследователь в Лос-Аламосской лаборатории. И их сын Крис. Тут реальный вклад сомнителен, ему в это время было лет тринадцать. Так же Тони Уорнок (Tony Warnock) и Том Бленко (Tom Blenko), понятия не имеем, кто они такие.   
В первой версии Хаскеля наконец появились современные гарды не требующие повторять имена:   
...
но и более общего средства для убирания повторяющихся имен в уравнениях Хаскель лишился.   
Гарды современного вида предложил Худак [Wadl25]. Худак предложил такие же гарды, ближе к паттернам, и для ЦФ-нотации: `[ x || x | x>0 <- xs ]` вместо `[ x | x <- xs, x>0 ]`. Но это предложение не приняли. Тут отличия от Миранды посчитали неприемлемыми [Huda91a].   
`as`-паттерны стали выглядеть как в современном Хаскеле и паттерны больше не irrefutable по умолчанию, если у АлгТД один конструктор - нужно писать `~`.   
Движение к современному Хаскелю было не только вперед. Haskell 1.0 лишился сечений. Одной из современных фич, которые уже были в черновике Haskell 88.12.   
...
Важное отличие от современного Хаскеля и Миранды осталось - в Haskell 1.0 выражение `where`, видимость не распространяется на гарды. Haskell должен был стать свободной Мирандой, а стал улучшенным Lazy ML.   

### Miranda Nouveau

И функциональные программисты оказались к такому не готовы. И открытая рассылка для обсуждения Хаскеля стала постепенно наполняться жалобами на отличия от Миранды. Функциональные программисты похоже ожидали, что единая не-Миранда будет отличаться от Миранды только формально, не по существу. Функциональные программисты в 90-ом году хотели бесплатную Миранду и не страдали от того, что язык, создаваемый комитетом, похож на язык, создаваемый комитетом.  
Жалобы были не от любителей тернеровского минимализма, если в Хаскеле есть что-то, чего в Миранде нет - это хорошо. Плохо, когда нельзя написать что-то в точности как в Миранде.   
Когда, например, на вопрос о том, почему гарды не в точности как в Миранде, Йонссон отвечал вопросом - а почему должны быть как в Миранде, а не как ALFL или Lazy ML? - ему объясняли: потому, что Миранда популярнее.   
В рассылке не протестовали особенно против лямбды, но не были довольны тем, что в Хаскеле нет сечений. Делали предположения о том, что сечения было сложно имплементировать и соглашались даже на сечения с wildcard как в HOPE, то есть на `(? + x)` или `(_ + x)` вместо `(+ x)`. Но Хаммонд заверил их, что имплементация сечений тривиальна, сечения не добавили в Хаскель только ради его упрощения [Hamm90a].   
Вопросы вызвало и возвращение к выражению `where` вместо свежего и удачного изобретения Тернера - `where` как части декларации.   
Пейтон-Джонс объяснял, что для того, чтоб иметь в языке и выражение `where` и тернеровскую конструкцию - для нее бы пришлось добавить другое ключевое слово, например `with`, но в большинстве случаев конструкции работали бы одинаково и небольшие различия между ними вызвали бы "путаницу".   
Кент Карлссон (Kent Karlsson), который уже поучаствовал в нашей истории в 84-ом году как выразитель пожеланий шведских ML-истов перед SML не-комитетом, написал множество пожеланий и для Haskell комитета. Среди прочих было и предложение [Karl90a] от сентября 90-го года добавить в Haskell к выражению-`where` еще и Тернер-`where`. Различать их с помощью отступов или точек с запятой. Как часть декларации работает `;where` и его аналог с соответствующим отступом. Но это предложение не нашло поддержки. Йонссон писал, что если вдруг понадобилось объявить что-то видимым для гардов - можно использовать вместо уравнений с ПМ `case`-выражение вместе с выражением `where`.   
В следующем, 91-ом году комитет работал над следующей редакцией описания Haskell 1.1 [Huda91]. Пейтон-Джонс стал третьим редактором описания. Арвинд, Майк Рив, Уайз и Янг переместились из списка авторов в список благодарностей. Новыми авторами стали Вилл Партейн (Will Partain) из Глазго, Мария Гусман и Джон Петерсон из Йеля. В список благодарностей также попали Кент Карлссон и будущие герои следующей части Корделия Холл (Cordelia Hall), работавшая в Глазго, Марк Джонс (Mark Jones), Марк Лиллибридж (Mark Lillibridge) и Сатиш Татте (Satish Thatte). Предполагалось, что изменения будут незначительными, но вышло иначе.   
Это обсуждение проходило все более открыто, все в большей и большей степени в открытой рассылке. В которую комитетчики, которые меньше заботились о секретности обсуждений (например Вадлер) отправляли тексты из закрытой рассылки, к неудовольствию тех, кто заботился о секретности больше (например, Пейтон-Джонс).   
В первоначальных планах было сделать Хаскель еще менее похожим на Миранду. Одной из проблем, на которую обратил внимание Карлссон было взаимодействие `where`-выражения с лямбдой. Такое вот выражение `if a then f else \x -> g x a where a = b` означало `if a then f else (\x -> (g x a where a = b))`, а программисты ожидали чего-то вроде `(if a then f else (\x -> g x a)) where a = b`. Худак предложил решить проблему сделав скобки обязательными.   
Пятого апреля 91-го Джо Фазель заключил, что набор конструкций Хаскеля 1.0 получился не очень удачным. Лямбды, `let in` выражения и `if then else` хорошо сочетаются и не требуют каких-то заключительных ключевых слов вроде `end` или `fi`. C `where`-выражением не все так просто. Так что решение очевидно, предлагает Фазель. Нужно убрать из Хаскеля лямбды и добавить вместо них сечения [Fase91a]. Лямбды не нужны: вместо `map (\x -> a*x + b) xs` всегда можно написать `map (f where f x = a*x + b) xs`.   
Брайан Боутел сделал альтернативное предложение: исключить из Хаскеля `where`-выражение и добавить вместо него `let in`.   
Вадлер пишет, что еще недавно он поддержал бы предложение убрать лямбды и добавить сечения. Но сейчас он пишет код в CPS-стиле и для этого лямбды просто необходимы. (Почему Вадлер пишет код в CPS-стиле? Об этом в следующей части). Замена `where` на `let in` кажется Вадлеру хорошей идеей, но слишком радикальным изменением для Haskell 1.1. Может быть для Haskell 2. Боутел возражает: такое изменение нужно делать как можно скорее - чем дольше откладывать - тем больше придется всего исправлять. Пейтон-Джонс из двух предложений выбрал `let in`, но считает, что на Хаскеле уже написано слишком много кода для такого изменения.   
В апреле состоялась очная встреча части комитета после встречи IFIP WG2.8 в Париже. Все участники очной встречи проголосовали за удаление `where` и добавление `let in`.   
После встречи комитета в париже Худак обсудил проблему с МакКвином. И МакКвин заявил, что вот как раз из-за таких проблем и удалили `where`-выражение из SML. Но, видимо, не сказал, что это стало одной из причин ML-раскола. Из OCaml, победившего SML в результате, выражение `where`, правда, позднее тоже удалили.   
Радовались единогласному решению недолго. Не участвовавшие в очной встрече проголосовали в основном против и в результате получилось восемь голосов за `let` и пять против. Для "синтаксического царя" Худака такой перевес показался недостаточным для принятия решения. Возникает, конечно, вопрос: если "синтаксический царь" не чувствует, что может принять решение с восемью голосами за и пятью - против, то в чем вообще смысл такой роли?      
Партейн написал значительную часть кода о котором говорил Пейтон-Джонс и отмечает, что взаимодействие лямбды с `where` ни разу не было проблемой. Это проблема выдуманная "языковыми юристами" и не имеющая практического значения. С другой стороны, `where` дает описание сверху вниз, что на практике удобно и читаемо. Сколько среди проголосовавших в Париже за `let` начинали с ФЯ в котором ничего лучше нет и это нанесло им неисправимые повреждения? - вопрошает Партейн.   
Партейн голосует: оставить все как есть. К тому же, предостерегает он, что подумают про Хаскель, если мы сделаем такое радикальное изменение в версии 1.1? Никто не будет им пользоваться после такого. Это возражение Боутел принял. Выглядеть такое радикальное изменение будет очень плохо.   
Худак не согласен, что `let` сложнее читается. Хьюз опасается, что никто не будет тратить время на язык который радикально меняется в каждой минорной редакции. Но у него есть решение: пусть в Хаскеле будет и `let` и выражение-`where`. Может решение и не очень красивое, но весь код продолжит компилироваться. Особой поддержки эта идея, правда, не получила.    
Боутел не был доволен таким решением. Он продвигал замену `where` на `let`, чтоб неоднозначностей было меньше, а взаимодействие между выражениями сделает еще больше неоднозначностей.   
Корделия Холл написала, что хоть ей и придется переписывать много кода, но ей не нравится идея, что так рано ломающие изменения в Хаскеле объявляют неприемлемыми. Язык еще в процессе проектирования.   
Хаммонд написал [Hamm91], что лично он всегда предпочитал `let`, но не хочет никого принуждать его использовать. И до этого обсуждения он считал что все кроме него предпочитают `where`. Сочетание выражений `where` и `let` может создавать неоднозначный код вроде такого:
...
Но удаление из языка `where` - серьезная проблема, так что предложение Хьюза стоит рассмотрения.   
Хаммонд считал, что нужно обратиться к опыту языка в котором есть `let` и `where`, например LML. Хаммонд отмечает, что в коде на LML `where`-выражение используется редко и предполагает, что это из-за необходимости использовать скобки, которые выглядят "уродливо". Обязательные скобки - как раз решение, которое предлагал Худак.   
Августссон, один из авторов LML и автор значительной части кода про который писал Хаммонд, тоже предпочитал `let` и подтвердил, что да, это из-за скобок. Но Августссон не видел почему бы не придумать `let` и `where`, которые лучше взаимодействуют. Это просто не было сделано для LML.   
Вадлер теперь предпочитал `let`, как и лямбды, из-за CPS-кода. Повторял, что раньше думал иначе, пока не стал писать CPS-код. Считал, что не стоит бояться радикально менять язык.   
Фазель написал [Fase91], что предпочитает `where` и уверен, что программисты обычно предпочитают `where` и в принципе когда им не навязывают объявление перед использованием. И программисты на ленивых функциональных языках предпочитают Миранду, а не LML. Но раз уж убирать из Хаскеля лямбду и оставлять `where` никто не хочет - Фазель предложил новое решение.   
27-го апреля 91-го года он предложил вернуться к тому, с чего Хаскель и начинался. Помните проект Вадлера в котором в языке есть и Тернеровский `where` как часть декларации и выражение? Фазель взял его за основу и внес несколько изменений. Добавить в Хаскель Тернеровский `where` как часть декларации и использовать для него ключевое слово `where`. Не `with` как почему-то было у Вадлера. Выражение `where` из Хаскеля убрать - это неудачная идея. Фазеля раздражает, что оно работает не как `where` в Миранде. И при добавленном вместо него `where` как в Миранде весь код переписывать не придется, разница есть только в некоторых случаях. Но для любителей выражений нужно добавить `let in`. Тернеровский `where` как часть декларации сочетается с лямбдами и `let in` без проблем. Может быть это выглядит избыточным. но избыточность это нормально для языков.   
И вот это предложение многие поддержали. Вадлеру понравилось, что теперь Хаскель сможет полноценно поддерживать и "плоский" стиль уравнений и "вложенный" стиль выражений. Тем более, что такую поддержку планировали с самого начала. Не понравилось то, что это выглядит так, как будто Хаскель состоит из двух языков. Но это не страшно. Но если мы делаем два языка в одном, предложил Вадлер, почему-бы не сделать в Хаскеле еще и гарды как в Миранде. Гарды были адаптированы к языку выражений. Но в рамках нового подхода гарды - конструкция "плоского" ФП, а для "вложенного" есть `if then else`. Каждое средство должно быть оптимальным для своего подхода.   
Пейтон-Джонс тоже поддержал предложение Фазеля. Он не любил выражение-`where` за то, что оно работает не как в Миранде. Само собой, добавление правильно работающего `where` он приветствовал. Но к предложению Вадлера о Миранда-гардах он отнесся равнодушно. Пейтон-Джонс был не против, но не более того. Достаточной поддержки для гард как в Миранде в комитете не нашлось.   
Худак написал, что необходимость иметь в Хаскеле и `where` и `let` очевидна. Вопрос только в том, сделать это так, как предложил Хьюз или как Фазель. Но что если рассмотреть третий вариант? И Худак предложил конструкцию из своего языка ALFL, в которой обозначено выражение-результат, а декларации могут быть как до него, так и после:
...
Брайану Боутелу это понравилось, но мало кому еще.   
Тем временем, к 8 мая 91-го Пейтон-Джонс сформулировал новую доктрину Хаскель-дизайна [SPJ91]. Хаскель, выглядящий как два языка в одном - это не проблема, а решение. Объединяя язык выражений с языком уравнений нужно не выбирать между конструкциями из одного типа ФЯ или другого. Нужно, наоборот, проследить чтоб для каждой задачи было по лучшему решению и из языка выражений и из языка уравнений. Пейтон-Джонс построил таблицу:

| Мир деклараций    | Мир выражений   |
| ------------------|-----------------|
| функции           | лямбды          |
| уравнения с ПМ    | `case`          |
| гарды             | `if then else`  |
| сигнатуры функций | аннотации типов |
| `where` Тернера   | `let in`        |

С точки зрения этого подхода, проблемы Haskell 1.0 c поддержкой "плоского" стиля уравнений очевидна и должна быть исправлена так, как предложил Фазель.   
Вскоре сформировался консенсус. Хаммонд был прав, большинство любило `where`, просто не тот `where`, который был в Haskell 1.0. И Haskell 1.1 получил популярный `where` и `let in`. Как и Advanced Intermediate, но другим путем. Там Миранду построили вокруг оставшегося от не-Advanced Intermediate `LET IN`, даже с отличающимся регистром от поздних ключевых слов из Миранды. Произошло это примерно в то же время, но сложно сказать раньше или позже. По крайней мере в открытой рассылке при обсуждении изменений Хаскеля Intermediate никто не вспоминал.   
Но Haskell 1.1 получил не только популярные `where` и `let`. Воодушевленный победой Фазель попытался добавить в новую редакцию еще пару конструкций. Одной из них были локальные декларации в list comprehensions. В это время это было обычное хаскельное объявление вида `x = y` без ключевого слова `let`.   
Локальные декларации обсуждались в открытой рассылке уже несколько месяцев. В январе 91-го их предложил один из важнейших героев следующей части Марк Джонс, но у них более длинная история, авторы Хаскеля обсуждали еще за год до того и они были даже имплементированы в одном из компиляторов Хаскеля, который, правда, в это время не был доступен для использования.   
В мае 91-го это нововведение поддержали Фазель и Худак. Конечно же Брайан Боутел посчитал эту конструкцию опасной. Что если программист случайно напишет `x == y` вместо `x = y`? Пейтон-Джонс согласился, что это - проблема, а Вадлер посчитал серьезной проблемой и стал противником предложения.   
Кент Карлссон предложил объявление современного вида `let x = y`. Это предложение Марк Джонс поддержал, но без особого энтузиазма, вариант без `let` нравился ему больше. Фазелю же предложение вовсе не понравилось, он утверждал, что с `let` конструкция выглядит не лучше, чем `x <- [y]`.   
Этим конфликтом воспользовался Худак, чтоб попытаться провести в Хаскель свой синтаксис гард `[ (y,y) || x | x>0 <- xs, y = f x ]` но не нашел нужной поддержки и в этот раз.   
Другой проблемой стал спор сторонников двух трансляций. Трансляция Марка Джонса предполагала, что `[ exp | ..., p = v, ... ]` означает `[ let p = v in exp | ... ]`. Это разрешает рекурсию. Трансляция Джона Петерсона предполагала, что `[ exp | ..., p = v, ... ]` означает `[ exp | ..., p <- [v], ...]`. В этом случае, если паттерн `p` не сопоставится, то соответствующее значение последовательности будет просто пропущено.   
Фазель не ожидал, что такое простое дополнение вызовет споры о синтаксисе и семантике, посчитал, что оно того не стоит и отозвал свою поддержку. В результате локальные объявления в ЦФ-нотации не попали в Haskell 1.1. Не ожидали? Разумеется, со временем они нашли дорогу в Хаскель, но не в тот раз.   
Второе дополнение которое продвигал Фазель были сечения. Интересно, что Фазель был противником сечений, когда они обсуждались как фича для Haskell 1.0. Обсуждение началось с недоразумения, Вадлер решил, что Фазель все продолжает пытаться заменить лямбды на сечения и стал протестовать. Но недоразумение быстро разрешилось. В этот раз сечения, конструкция из языков уравнений, должны были дополнять конструкцию из языков выражений - лямбду. Вадлер стал сторонником предложения.  
Худак был против сечений как раз потому, что они дополняют лямбды. Он не считал, что дополнение такое уж ценное. Пейтон-Джонс и Джон Петерсон согласились поддержать идею, если Фазель опишет новый синтаксис Хаскеля с сечениями и в этом описании не будет особых проблем. Что Фазель сделал.
Так Боутел и Фазель определили современный вид Хаскеля. Худак же, хоть и был "Синтаксическим Царем", но не смог провести много собственных синтаксических идей. Может быть и к лучшему. Синтаксис Хаскеля определили синтаксические бояре.
...
В результате Haskell 1.1 стал той самой не-Мирандой которую и ожидали от его авторов с самого начала. Что, конечно же, не осталось незамеченным Тернером.   
Возвращаясь из Брюсселя, Тернер пообщался с Хьюзом. И тот рассказал Тернеру про изменения в Хаскеле, о том, что там теперь будет `where` как в Миранде.   
Так что в конце июля 91-го Тернер написал Пейтон-Джонсу письмо [SPJ91a]. Понятное дело, пишет Тернер, я не видел новую редакцию описания, но не сделает ли вот это вот изменение `where` Хаскель слишком уж похожим на Миранду? Особенно с учетом уже имеющихся сходных деталей. Тернер выражает обеспокоенность, что это может вызвать "сложности", которые - уверен Тернер - обе стороны предпочтут избежать.   
Тернер не уверен, что есть смысл подключить его к редактированию описания до того, как оно будет опубликовано. Но Тернер считает нужным по крайней мере "сигнализировать" о наличии "потенциальных сложностей".   
Пейтон-Джонс написал в ответ, что описание уже готово и что он не согласен, что изменения делают Хаскель похожим на Миранду, очевидно, что языки очень разные. Пейтон-Джонс переслал письмо Тернера и свой ответ в рассылку, в которой разгорелось то самое обсуждение, упомянутое уже в нашей истории.   
Худак написал, что он категорически против включения Тернера как наблюдателя в процесс разработки Хаскеля. И против того, чтоб менять в Хаскеле даже самые мелкие детали по указке Тернера. Мы в свое время предлагали Тернеру войти в комитет, напоминает Худак. И Тернер сам отказался. Худак перечисляет прошлые попытки Тернера помешать Пейтон-Джонсу, Вадлеру и Берду и заключает, что Тернер не мог и не сможет создать никаких сложностей. И все возникшие сложности Пейтон-Джонс, Вадлер и Берд создали для себя сами, потому что были достаточно добры для того чтоб попытаться задобрить Тернера. Худак, похоже, дает понять, что сам он для этого не достаточно добр.   
Нихил сформулировал свою позицию по этичности создания вариантов Миранды. Вадлер и Пейтон-Джонс выразили сомнения, что Тернер сделает что-то еще, помимо написания письма. И если вдруг и сделает, то тогда и нужно будет об этом беспокоиться. Тернер пишет, что не читал новую редакцию описания, ну так Пейтон-Джонс пошлет ему копию. Тернер почитает описание - предполагает Вадлер - и впечатление об излишней схожести языков пройдет.   
Так или иначе, похоже, что "потенциальные сложности" действительно так никогда и не материализовались.   
На этом триумфальное шествие HOPE-фикаци по языкам эдинбургской программы закончилось. Великое уравнивание завершилось и произвело семейство похожих языков уравнений. От самобытности ранних синтаксических экспериментов мало что осталось. Но разнообразие сохранилось в другом. Языки, про которые мы рассказали в этой части, были большим, чем просто воспроизведением HOPE раз за разом. Их создатели нашли решения для многих проблем, которые авторы HOPE сформулировали, но решить не смогли. Об этом мы расскажем в следующей части. Но не раньше, чем после короткого обзора программ родственных Эдинбургской программе, в которых HOPE-фикация не была так успешна.   

Последние надежды
-----------------

Авторы практически каждого из вариантов первоначальных языков Эдинбургской программы, будь то ML, NPL или SASL, добавляли в свои языки средства из остальных. Пока они не стали похожи друг на друга. Начиная с HOPE и заканчивая Haskell.   
Но в нулевой части мы ввели Эдинбургскую программу как одну из множества родственных, взаимовлияющих исследовательских программ. Пришло время посмотреть на успехи HOPE-фикации на этих направлениях. Ясно, что HOPE-фикация была не столь успешна за пределами Эдинбургской программы, иначе мы писали бы историю не Эдинбургской программы, а какой-нибудь другой программы, результатом которой было появление того, что мы считаем современным функциональным языком. Так что точнее сформулировать вопрос так: насколько неуспешной была HOPE-фикация языков родственных программ и как современный ФЯ мог бы выглядеть, сложись события иначе.    

### Algol 84   

В комитете разработчиков Algol 68 были партии, которые хотели сделать из него полноценный ФЯ и которые хотели добавить в него параметризованные типы и модули. Они, по большему счету, потерпели поражение в Фонтенбло, но на этом история не закончилась. Algol 68 существенно повлиял на множество языков.   
Не на первые языки Эдинбургской программы. Мы выяснили, что для них главным влиянием был CPL/ISWIM. Но если бы CPL не было, можно предположить, что его место мог бы занять и Алгол 68. По крайней мере, такой вывод вполне можно сделать из результатов влияния Алгола 68.   
Не для всех авторов языков, на которых Алгол 68 повлиял, он был примером для подражания.  Некоторые произошедшие от Алгол 68 языки были восстанием против Алгола 68 и многих несбывшихся планов его авторов. CLU и ADA были восстанием против функционального программирования, но осуществили мечты о параметризованных типах и модулях. Паскаль был восстанием и против того, и против другого.   
Но некоторые, не самые популярные произошедшие от Algol 68 языки, осуществили самые смелые мечты Бауэра, Самельсона и Бекича. Но сделало ли это HOPE из этих языков? Давайте выясним.   
Автор Poly какое-то время планировал добавить фичи из HOPE, пока не перестал планировать и не стал разработчиком HOPE-образного языка Standard ML.   
Про Ponder писали [Stoy85], что управляющие конструкции в языке можно определять самостоятельно. Что можно определить в библиотеке операции над типами `IsType`, `\./` и `Dot`, определить селекторы и конструкторы `in_1`, `is_1`, `in_2`, `is_2` и так далее. И использовать эту библиотеку для того, чтоб объявлять композитные типы   

```haskell
Capsule Type Lex == Expr \./   -- literal (int, char or string)
                    String \./ -- alphanumeric name or punctuation
...
Объявить в библиотеке операторы `In`, `||` и `Out`, скобки `Case` и `Esac` и кое-как разбирать объявленные выше композитные типы:
...
На Ponder даже писали компилятор с использованием всех этих средств. Потом, конечно, разработчики Ponder просто стали разработчиками HOPE-образного языка Haskell.   
Видимо, превратить ISWIM/CPL в очередной HOPE было все же попроще, чем Algol 68.   
Разработчики языка Russell не стали добавлять в него фичи из HOPE, как и разработчики ML из того же университета - Корнелла.   
Другим центром развития Algol 68, в котором из него не особенно хотели сделать HOPE был Мюнхен. В Мюнхене продолжалась работа, которую Бауэр и Самельсон начали в 70-е, после поражения Бекича в Фонтенбло. Фактически эта работа была воспроизведением работы Бурсталла. В Мюнхене делали язык алгебраической спецификации CIP-L [Baue85] и систему переписывания кода CIP-S [Baue87]. CIP-L был скорее не языком, а общим представлением для языков, первыми из которых были функциональные версии Алгола 68 и Паскаля, которые теперь назывались ALGOL 84 и PASCAL 84. Мюнхенцы, разумеется цитируют ту же статью с шутками Кнута, что и описание Orwell. Эти Алголы - полноценные ФЯ с композицией функций `o` в библиотеке:  

```
(funct(n)r f , funct(m)n g) funct(m)r :
    (m x) r : f(g(x))
```

и полноценные языки спецификации со всеми полагающимися уравнениями описывающими спецификацию - "законами".   

```
type LIST == (sort m)
                m, list, nil, null, cons, head, tail :
    sort list ,
    list nil ,
    funct (m, list) list cons ,
    funct (list) bool null ,
	  funct (list) m head ,
    funct (list) list tail;
    laws list l, m x, m y :
        null(nil) == true ,
        null(cons(x, l)) == false ,
        head(cons(x, l)) == x ,
...
Как видите, законы могут быть вполне исполняемыми, но не исполняются, нужно все имплементировать вручную

```
structure L == (sort m)
                   list, nil, null, cons, head, tail :
    mode list = nil | cons(m head, list tail) ,
	  funct null == (list l) bool : l is nil
end of structure
```

Ну, не все, имена полей конструкторов - уже готовые функции, в отличие от Algol 68. Также, в отличие от Algol 68, можно объявить рекурсивный тип не разбивая рекурсию ссылками или абстрактными типами данных. Абстрактный тип данных все равно нужен для параметризации.  
И мы не видели какой абстрактный тип нужен был для параметризации `map`, так что обычного примера не будет. Это не значит, что написать `map` на Algol 84 было невозможно, в этой главе мы еще увидим похожие примеры на похожих языках, но восстановить работающий пример без работающей имплементации в этом случае не легко.   
Если не считать того, что параметризация типов осуществляется с помощью параметризуемых модулей, как в CLEAR из нулевой части нашей истории, то алгебраические типы данных в Algol 84 (и в Pascal 84) полноценные. Но в CIP "алгебраическими типами" называются эти самые модули, которые сегодня называют обычно абстрактными типами данных. То, что мы называем сегодня алгебраическими типами в CIP называют как в Алголе 68 "модами" и "непосредственными определениями", как называют в некоторых языках спецификации.   
Такие языки спецификации обычно позволяют получать непосредственное определение прямо из уравнений, описывающих законы. Но не в случае CIP.   
Моды в Algol 84 больше похожи на современные алгебраические типы - суммы произведений - чем на маккартиевские моды в Algol 68 (CPL, VAX ML) в которых суммы отдельно, а произведения отдельно. Но вот способ их разбора похож на современный меньше. В ревизии Algol 68 было `case`-выражение с примитивным одноуровневым паттерн-матчингом, но в Algol 84 только предикаты `переменная is конструктор` используемые в обычных `if`, вроде того, что придумывал Бурсталл до того, как его раскритиковал Попплстоун.   
В очередной раз Бауэр продемонстрировал, насколько необычным может быть представление о том, что должно быть в функциональном языке. Не смотря на все те же предпосылки, что были у Бурсталла и Дарлингтона: правила переписывания, языки уравнений для описания спецификации, идеи из первых ФЯ 60-х и 70-х, и даже знакомства с основными работами Дарлингтона и Бурсталла, делать HOPE из Algol 68 Бауэр не хотел и не сделал.   
Языки спецификации и система для переписывания спецификаций на них в исполняющийся код на Паскале и Си были имплементированы до того, как Бауэр ушел на пенсию в 89-ом. Что-то работало не позднее 87-го.   
Самельсон умер в 1980-ом году, так что не дожил до сколько-нибудь работающих имплементаций полноценных функциональных языков, произошедших от Алгола 68. Компиляторы Poly, Ponder, Russell и Algol 84 появились позже. Но Бауэр дожил до 2015 и если не пережил функциональное программирование вообще, то, по крайней мере, пережил его расцвет.   
Но если Бауэр не хочет делать HOPE - не значит, что никто не хочет. В середине 90-х в Университете Неймегена возникла проблема. Они использовали CIP-L для преподавания курса по трансформации программ. И в середине 90-х им не удалось найти подходящую работающую имплементацию CIP. И в университете решили имплементировать CIP-L с помощью системы ASF+SDF. SDF - это язык для описания синтаксиса, а ASF язык переписывания результатов разбора синтаксиса, с уравнениями и паттерн-матчингом.   
И в заново имплементируемом CIP-L не хватало уравнений и паттерн-матчинга, так что в итоге имплементировали не CIP-L - Algol 84, а Clean с элементами CIP-L под названием DISCIPLEAN [Bran95].   
Даже если и засчитать это за HOPE-фикацию Algol 68 - произошла она с не особенно важным развитием его не особенно важного развития и существенно позже, чем HOPE-фикация обычно происходила.   

### Scratchpad

В нулевой части нашей истории, в начале 70-х, исследовательский центр IBM в Йорктаун Хайтс был важным и влиятельным центром развития функционального программирования. Эта важная роль осталась в прошлом. Но развитие системы компьютерной алгебры SCRATCHPAD продолжалось и теперь ее разработчики заимствовали идеи у более прогрессивных исследовательских программ.   
Ричард Дженкс подумывал над новой версией SCRATCHPAD с 73-го года [Jenk86]. В 77-ом поменялся руководитель проекта с Джима Грайсмера (Jim Griesmer) на Дэвида Юна (David Yun), который согласился на переписывание Scratchpad с нуля.   
Новую систему писали на LISP/370 (с 1984 назывался LISP/VM). Точнее, как это обычно бывает при разработке систем компьютерной алгебры, не на Лиспе непосредственно, а на алголо-подобном языке Boot, имплементированном как набор макросов.   
...
Главным нововведением должна была стать типизация. И с системой типов начали экспериментировать с разработки MODLISP, типизированного LISP с "модами".   
Летом 1978 к проекту подключился Джеймс Дэвенпорт (James Davenport), который и имплементировал первоначальную версию MODLISP.   
Типы обсуждали с Юном, Артуром Норманом (Кембридж, тот, кто познакомил Пейтон-Джонса с ФП и позднее поучаствовал в разработке Haskell) и Джошем Коэном (Josh Cohen), который потом работал над проектом пять лет.   
В следующем году Дженкс и Коэн имплементировали MODLISP полностью. Но у такого подхода к типизации Scratchpad была короткая жизнь.   
В сентябре 79-го Джеймс Давенпорт возвращается и предлагает систему, придуманную им под впечатлением от "категорий". Параметризованные абстрактные типы - конструкторы "доменов" - назвали "функторами" и это название для них мы слышим не последний раз.   
В конце 79-го Давенпорт и Дженкс начали имплементацию второго прототипа на основе MODLISP
с категориями и функторами. В этот раз уже язык программирования развивали достаточно долго, чтоб имплементировать Scratchpad-подобный синтаксис. С конца лета 80-го заработали вывод типов и компилятор функторов.   
Scratchpad - название системы, а сам язык в это время называют spad по расширению файлов с кодом. Или, чтоб явно обозначить, что речь идет про новую, типизированную версию - newspad.   
К концу 80-го авторы newspad все еще не были удовлетворены частями позаимствованными из языков алгебраической спецификации.   
Барри Трагер (Barry Trager) присоединился к группе в январе 81-го, он изучил литературу по языкам программирования, например по CLU и Russell. И авторам spad понравилась "первоклассность", то, что в Russell язык спецификации не специальный язык - а функции. Над идеей поработали, посоветовались с Джимом Тэтчером (Jim Thatcher), Джерри Арчибальдом (Jerry Archibald, IBM Research), Д. Эрихом (D. Ehrich, Университет Дортмунда), Дэвидом Сондерсом (David B. Saunders, RPI) и Ричардом Зиппелем (
Richard Zippel). И результат получился довольно неожиданным при таком числе советчиков. Более-менее обычные конструкции из языков алгебраической спецификации
...
после расселизации стали выглядеть так:

```
GradedExt(R: Ring, E:OrgMonoid): Ring with
    degree:  $ -> E
    content: $ -> R
...
Только Дэвид Бартон (David Barton) придумал для первоначальной версии 18 ключевых слов, все они после расселизации были или убраны из языка или заменены на функции.   
Осенью 81-го Дженкс начал третье и пока последнее переписывание имплементации с нуля.   
Еще в 83-ем степень готовности нового компилятора все еще была неудовлетворительной. Не удовлетворяла низкая производительность из-за имплементации "доменов", и это не последний раз, когда мы будет рассказывать об этой проблеме, правда уже не в spad.   
В 83-ем Виктор Миллер (Victor Miller) применил наработки разработчиков компилятора ADA, которые решали похожие проблемы оптимизации параметризованного кода. В конце 84-го к группе присоединился Стивен Уатт (Stephen Watt), который завершил работу, начатую Миллером.   
Группа разработчиков Scratchpad 2 до сих пор состояла из 2-3 человек, но в 84-ом году выросла до семи.   
Летом 85-го Уатт решил, что Scratchpad в достаточной степени зрелый язык программирования, чтоб имплементировать его на самом себе.   
Не позднее ноября 87-го [Spad87] IBM наконец дала возможность попользоваться Scratchpad за пределами IBM. Университет мог заплатить $1000 ($2900 в 2026) и получить программу. Минимальные требования - 16Мб памяти на мейнфрейме IBM или 8Мб на рабочей станции IBM PC RT, той самой, которую не стали покупать. Мейнфреймы IBM хорошо покупали, но как мы выяснили в нулевой части, история сложилась так, что покупали обычно не функциональные программисты. Из важных для нашей истории университетов Scratchpad 2 получил университет Чалмерса. Конечно, довольно поздновато для того, чтоб почерпнуть какие-то полезные для разработки ФЯ идеи оттуда.   
В IBM уже не хотели скрывать Scratchpad от мира, а хотели чтоб система работала на более распространенных машинах. Так что работающий в IBM Research Тим Дейли (Tim Daly) портировал [Daly2003] Scratchpad на Common LISP. Что было вполне выполнимой задачей потому, что Scratchpad был в основном написан на самом себе и на собственном языке - наборе макросов. И такие наборы макросов не первый и не последний раз в нашей истории помогали лисперам переехать с одного Лиспа на другой.   
Но, как мы выяснили, с имплементациями Common LISP дела обстояли не особенно хорошо. Мы уже писали про то, как IBM пыталась помочь авторам Spice Lisp/CMUCL развивать его, но он все еще работал только на одной из худших Lisp-машин и на той самой рабочей станции IBM которую не покупали.   
Была, правда, еще одна условно-свободная имплементация Common LISP, которая может не была такой хорошей как CMUCL с технической точки зрения, но зато компилировала в C и потому могла быть использована для того, чтоб Scratchpad заработал на многих популярных машинах. IBM получил у Киотского университета разрешение распространять эту имплементацию - AKCL - вместе со Scratchpad. Так что разработчик AKCL Билл Шелтер получил деньги на развитие AKCL не только от "энергетиков", а еще и от IBM, а Дейли поработал немного и над AKCL.   
С января 86-го над Scratchpad 2 работал наш старый знакомый по нулевой части Уильям Бердж, который занимался своим любимым делом - разрабатывал библиотеки ФВП для работы со стримами [Burg87] :
...
И ФВП - нормальное явление в spad библиотеках. Наш обычный пример выглядит так и, в основном, является реальным кодом [ListSpad] с сокращениями:
...
Чтоб написать `map` нужен модуль/АТД параметризованный двумя типами
...
ListFunctions2(A : Type, B : Type) : public == private where
    ...
    private ==> add
        map(fn : A -> B, l : LA) : LB ==
            empty?(l) => empty()
            cons(fn(first(l)), map(fn,rest(l)))
```

В реальном коде для имплементации `map` и похожих функций чаще использовали ЦФ-нотацию `[fn(a) for a in l]`.     
В языке довольно оригинально выглядящее [Daly2003] лямбда-выражение:   

```
map(x +-> x + y, [1,2,3])    
```

В этом примере, правда, не хватает многих конструкций, ожидаемых от ФЯ конца 80-х. Как мы выяснили еще в нулевой части, уравнения с паттерн-матчингом были в Spad еще до влияния языков алгебраической спецификации. И в новой, типизированной версии они сохранились. Можно написать, как и на HOPE-фицированных языках 
...
или, чтоб продемонстрировать другой вариант гард:
...
И гарды в этих примерах нужны потому, что последовательность уравнений не имеет значения, как и в HOPE. Но сходство обманчиво. Паттерн матчинг работает [Daly2003] только для констант: чисел, строк, логических значений. Даже встроенные конструкторы вроде списков так разбирать нельзя.   
Даже на Boot можно было написать `x is [a, :b] => b`, в Spad этому соответствовал код [Bake26] :
...
При этом в Scratchpad есть правила перезаписи с выразительным паттерн-матчингом, но все это работает только как средство мета-программирования, для преобразования выражений на самом Spad:   
 
```
logRule := rule log(x) + log(y) == log(x * y)
```

Такой код объявляет функцию, которая может быть применена к значению типа Expression, но для прочих конструкторов такие средства недоступны.   
Нет в spad и алгебраических типов. Типы там [Daly2003] больше похожие на маккартиевские, как в Алголе 68 и VAX ML: юнионы и рекорды. Рекорды более-менее обычные, а юнионы такого вот вида:

```
Union(selector1 :type1 , selector2 :type2 , ..., selectorN :typeN )
```

которые разбираются так:

```
sayBranch(x:Union(int:Integer,str:String,flo:Float)):Void==
  output
    x case int => "Integer branch"
    x case str => "String branch"
    "Float branch"
```

`case` тут не `case`-выражение, а операция сравнения с конструктором, вроде `is` из Алгола 84 выше, на месте условия в обычных CPL-образных условных выражениях.   
И для описания рекурсивных типов данных вроде списков с помощью юнионов и рекордов нужно использовать абстрактные типы данных, как в LCF/ML. В библиотеках spad так списки не описывают, но описывают в примерах на более поздней версии spad под названием Aldor [Aldor2002] :

```
define ListType(S: OutputType): Category == with {
    cons: (S,%) -> %;
    first: % -> S;
    rest: % -> %;
    empty: %;
...
List(S: OutputType): ListType(S) == add {
    Rep == Union(nil: Pointer, rec: Record(first: S, rest: %));
import from MachineInteger, Boolean, Rep;
    cons (s:S,l:%):% == per(union [s, l]);
    first(l: %): S   == rep(l).rec.first;
    rest (l: %): %   == rep(l).rec.rest;
    empty: % == per(union nil);
    empty?(l: %):Boolean == rep(l) case nil;
}   
```

И в этом случае получается, что не смотря на все те же исходные компоненты: средства из языков алгебраический спецификации, правила перезаписи, какой-никакой но паттерн-матчинг, идеи из первых ФЯ и даже не смотря на основного популяризатора этих идей - Берджа - в команде разработчиков, HOPE опять не получилось. Правда, в отличие от работ Бауэра, в Йорктаун Хайтс HOPE не получилось изобрести независимо. По всей видимости, ранние контакты с исследовательской программой Бурсталла существовали, но прервались в середине 70-х.   
И когда в начале 90-х HOPE-фицированные функциональные языки стали известнее, разработчики spad решили, что может это не такая плохая идея.   
В начале 90-х Scratchpad назывался Axiom, а новые версии spad назывались A#. И в 94-ом году Дженкс и Трагер решили [Jenk94] добавить в систему еще один язык B# (произносится B-natural), с функциями, которые можно определять так:   
...
и так `sum == fold.+.0`.   
По описанию авторов, язык должен был быть "похож на Haskell". Но эти планы не осуществились, в это время IBM уже забросил Scratchpad как исследовательский проект [Page2005] и предпочитал видеть его как коммерческий продукт Axiom. И у коммерческого продукта были более приоритетные задачи, чем HOPE-фикация.   


### Переписывание по правилам

Майкл О'Доннел (не перепутайте с Джоном, имеющим отношение к T) проработал над своим языком уравнений больше десятилетия. И наконец, не позднее 87-го года, под его руководством Роберт Странд (Robert Strandh) написал компилятор, который продемонстрировал приемлемую производительность. Приемлемой скоростью О'Доннел посчитал скорость кода, который генерировал компилятор Franz LISP. Да, пишет О'Доннел, производительность Franz LISP не считается особенно хорошей, но приемлемой-то ее вполне можно считать. И компилятор его языка уравнений генерирует код, который "лучше для многих примеров" [O'Do87].   
О'Доннел видел [O'Do98] свой язык, и вообще языки описания правил перезаписи, как часть обширного семейства "логических языков уравнений", в которое входят также функциональные языки и языки алгебраической спецификации.   
По всей видимости, такое представление о едином семействе было не только у О'Доннела. Например, в 18-ом году прошло [Gara18] четвертое соревнование языков переписывания. В котором функциональный язык OCaml занял третье место, язык алгебраической спецификации Maude - второе, а первое место занял функциональный язык Haskell. Такой результат мог бы выглядеть довольно мрачно для языков переписывания по правилам, если не считать функциональные языки и языки алгебраической спецификации языками переписывания по правилам. Но похоже, что такое представление о триедином сообществе языков уравнений - односторонне.   
По мнению О'Доннела из этого семейства его язык выделялся бескомпромиссностью. Но пока О'Доннел доказывал, что бескомпромиссный язык уравнений может иметь "приемлемую" производительность он обнаружил, что бескомпромиссный язык уравнений - возможно не тот язык уравнений, на котором хотели бы писать программисты. Они не особенно оценили плюсы этой бескомпромиссности, например возможность применять функции слева от `=`. Зато посчитали важными минусы бескомпромиссности, а именно - невозможность использовать перекрывающиеся паттерны. Правда, эти открытия не изменили дизайн языка уравнений. Не стал язык О'Доннела более похожим на HOPE и в других аспектах.   
Работы по имплементации языка О'Доннела продолжались и в середине 90-х. Странд работал, например, над анбоксингом и сравнивал [Stra94] свои труды с передовыми работами имплементаторов Haskell и CAML. Но работал он над оптимизацией все того же "гипер-чистого" M-Лиспа первого порядка из 70-х.   
Справедливости ради нужно заметить, что языки правил перезаписи позволяют добавить много интересного в язык без необходимости изменять что-то в его имплементации. Это интересное может быть правилами перезаписи в библиотеке. Но и особых следов такой деятельности по расширению языка О'Доннела мы не видели. А те, что видели относятся к первым экспериментам, которые О'Доннел с коллегами посчитали неудачными.   
Но у О'Доннела были последователи, которые создали языки описания правил переписывания и языки программирования более похожие на языки Эдинбургской программы.   
Первоначально О'Доннел разрабатывал свой язык как язык описания исполняемой спецификации-интерпретатора других языков вроде Лиспа или расширенного лямбда-исчисления. И код предполагалось писать уже на этих языках. Но оказалось, что производительность кода, получаемого при таком подходе "приемлемой" не была. Приемлемым было непосредственное использование языка уравнений для написания кода. И О'Доннел переключился на этот подход. Но последователи О'Доннела развивали оба направления.   

#### ASF+SDF, Stratego, Rascal

Важным представителем первого подхода был ASF+SDF, упомянутый выше и использованный как раз так - для получения имплементации из спецификации диалекта CLEAN с элементами Algol 84 под названием DISCIPLEAN.   
ASF+SDF объединял язык алгебраический спецификации ASF [Berg89] и язык описания синтаксиса SDF [Heer89] над которыми работали по крайней мере с 89-го года в Амстердаме.   
Со временем код на ASF+SDF стал больше походить на код на языках Эдинбургской программы, чем код на языке О'Доннела. В библиотеке ASF+SDF есть `map`. SDF-файл [SDFMAP] описывает модуль:

```
module functions/Map[From To]

imports containers/List[From] 
imports containers/List[To]

exports

context-free syntax

map -> ((From => To) List[[From]] => List[[To]])
...
hiddens sorts From To List[[From]] List[[To]]
variables
...
А ASF-файл [ASFMAP] правила перезаписи, имплементацию:
...
[] map(f LF) = cons(f(head(LF)),map(f tail(LF)))
     when LF != []   
```

На смену ASF+SDF пришли [Gara18] Stratego [Viss99] и Rascal [Bos11]. Stratego, появившийся в конце 90-х, развивался совсем не в направлении функционального языка уравнений с ПМ.   
В библиотеке [STRMAP] этого языка `map` выглядит так:

```
map(s) = rec x([] + [s | x]) 
```

Rascal [Bos11] - это не тот язык Алексис Кинг, который Racket + Haskell. Это очередной амстердамский язык правил переписывания, появившийся, судя по датам в исходниках, в 2009. Этот язык правил переписывания подошел к языкам Эдинбургской программы очень близко. Самое бросающееся в глаза отличие - обилие обязательных аннотаций типов и их Алголовый стиль.   
Но были времена до и после основных событий HOPE-фикации, когда обязательные аннотации типов были более нормальным явлением в языках Эдинбургской программы.    
На Rascal можно написать код, очень похожий на код на языке Эдинбургской программы [Vinj13]
...
Но код функции `map` в библиотеке [LISTRSC] написан не в таком стиле, но с использованием другой фичи языков Эдинбургской программы - ЦФ-нотации.    
...
В Rascal есть еще и версия этой нотации не для генерации списков, а для их сворачивания:
...
Но на Rascal также можно написать (и часто пишут) код, который не особенно похож на код на языках Эдинбургской программы:
...
Rascal - продукт совсем другой эпохи и совсем другого процесса, чем HOPE-фикация, о которой мы рассказываем в этой части. Типичный язык этой части получается, когда авторы какого-нибудь CPL 70-х (например LCF/ML) добавляют в него уравнения с ПМ. Rascal получен авторами языка уравнений с ПМ добавлением в него CPL 2000-х (в данном случае, наверное Java 5).   
И мы забрались так далеко от тех времен, когда HOPE-фикация, в основном, происходила потому, что все направление переписывания по правилам не сыграло больше заметной роли в нашей истории, и в этой части мы с ним прощаемся. Но не раньше, чем расскажем о языке, о котором некоторые читатели скорее всего слышали.   

#### Q, Pure

Представителем второго направления был Альберт Греф (Albert Gräf, бывает записан как Graef). Греф занимался эффективной имплементацией паттерн-матчинга для переписывания термов с 1990 [Grae91]. Он вдохновлялся трудами О'Доннела и его коллег и решил делать из правил перезаписи не конструктор имплементаций вроде ASF+SDF, а практичный язык програмирования Q (eQuational). Не более известный Q, который больше похож на K, J и APL. Полагаем, что Греф больше известен как автор своего следующего языка.  
В марте 91-го Греф написал [QNEWS] первый прототип имплементации Q версии 1.1 для MS-DOS на Turbo Pascal. В марте следующего 92-го года была готова версия 1.2 - на C для UNIX.   
Самая ранняя версия, исходный код которой мы видели - 1.7, опубликованная [Graef94] в 94-ом году.   
Библиотечная функция `map` там выглядела так:
...
так выглядели функции, определяемые с помощью применения комбинаторов:   
...
Обратите внимание на гарды в конце правых частей уравнений:   
...
Существенно позже Греф добавил в Q и гарды в началах правых частей, но с теми же ключевыми словами.   
Греф читал книгу Берда и Вадлера о функциональном программировании на Mirandorwell, книгу Пейтон-Джонса об имплементации ФЯ, статьи Августссона о том же и хотел сделать из Q ФЯ 80-х, но на это у него ушли все 90-е и еще немного. Более-менее зрелой и стабильной версией Греф называет 2.1 от декабря 2000. В этой версии появилось `let` и `case` выражения и паттерн-матчинг в лямбде. В версии 3.1 (апрель 2002) появилось Тернеровское `where`, паттерн-байндинги и `_`-вайлдкарды в ПМ. А `let`-выражение убрано за ненадобностью.   
Сам Греф называет свой язык функциональным. Но нужно отметить, что в случае языка уравнений, задающих правила переписывания, не так просто определить, является ли он функциональным. Может быть, в соответствующем сообществе уже отгремели все необходимые споры и все решено, но эти дискуссии неизвестны нам, концентрировавшимся больше на истории функциональных языков Эдинбургской разновидности.   
В чем сложность определения? Греф позиционирует свой язык как в первую очередь более-менее обычный язык программирования и, с помощью оборачивания сторонних библиотек, во вторую очередь язык для системы компьютерной алгебры. Не как конструктор для имплементации языков. Но языки переписывания по правилам все равно такими конструкторами являются. И значительная часть функционала, доступному пользователю языка Q из коробки была имплементирована именно так. Большую часть времени своего существования интерпретатор не поддерживал лямбды и альтернативные способы определения локальных функций со свободными переменными вроде `let`. Лямбды были реализованы как библиотечные функции, описывающие трансляцию лямбд в тернеровский набор комбинаторов, их оптимизацию и исполнение. И конструкции вроде `let` определены через эти лямбды.
Разумеется, определение лямбды в библиотеке - впечатляющая демонстрация мощи языка. Но для того, что позиционируется как "функциональный язык", встроенная поддержка лямбд может иметь свои сильные стороны. Улучшенную производительность, например. И в версии 7.1 от июня 2006-го интерпретатор стал поддерживать лямбды.
Библиотечная лямбда обозначалась словом, но встроенные лямбды были как в LCF/ML.

```
map (\X. X + Y) [1..3]
```

Но полная HOPE-фикация Q не состоялась, язык остался нетипизированным.   
Можно было декларировать структуры, похожие на АлгТД

```
type Stream = const nilstream | special binstream X Xs;
```

и использовать как АлгТД конструкторы

```
map F nilstream = nilstream;
map F (binstream X Xs) = binstream (F X) (map F Xs);
```

а имя "типа" для того, чтоб проверить этот набор конструкторов во время исполнения "тип-гардом":
    
```
unzip Xs:Stream = (map fst Xs, map snd Xs);
```

Но никакой статической проверки.   
Добавление поддержки лямбды в интерпретатор произошло только в последние годы развития языка. Летом 2008-го Греф посоветовал [Grae08] посетителям сайта языка Q обратить внимание на новый язык Грефа - Pure. Который, к тому же, еще и имплементирован с помощью LLVM JIT-компилятора и в десятки раз быстрее интерпретатора Q.   
Этот Pure - скорее всего именно тот Pure, о котором вы и подумали. Но, скорее всего, и  об этом языке многие читатели узнают впервые.   
Код на Pure [Grae21] выглядит еще более схожим с кодом на языках Эдинбургской программы:

```
map f [] = [];
map f (x:xs) = f x : map f xs;

map (\x -> x + y) (0..3) with y = 1 end;
```

Но и в этом случае сходство обманчиво.   
В pure более развитая чем в Q система декларативных проверок времени выполнения, которая выглядит иначе чем в Q   

```
type mylist nil | mylist (cons x xs);
```

Но как и в Q статической проверки типов нет.   
Нетипизированные функциональные языки уравнений с ПМ были популярны в 80-е, в основном благодаря Тернеру, но не пережили HOPE-фикации. Сегодня что-то похожее на них существует, практически, только как Pure. Можно допустить, что обычный ФЯ мог бы быть похожим языком, если бы ML не состоялся.   

### Алгебраическая спецификация

В 70-е годы Бурсталл и Гоген совместно разработали язык алгебраической спецификации CLEAR, но над его имплементацией и развитием в дальнейшем независимо работали две группы: эдинбургская и американская. В Эдинбурге в результате разработали и имплементировали первые современные ФЯ. Но такой исход не был предопределен. Американская группа Гогена работала именно над языками алгебраической спецификации, а не делала из них функциональные языки. Гоген с коллегами последовательно приближались к реализации CLEAR, оценивали свои результаты в основном с точки зрения успешности попыток реализовать и улучшить эти амбициозные планы из 70-х.   

#### OBJ

В нулевой части мы оставили эту группу в конце 70-х, когда в Калифорнийском университете в Лос-Анджелесе (UCLA) Тардо и Гоген имплементировали первое приближение под названием OBJ0. К 79-ому они имплементировали следующую версию - OBJT.   
В дальнейшем работы продолжались в SRI (Stanford Research Institute), куда ушел работать из Эдинбурга Бойер и где Фурукава раздобыл распечатки исходного кода интерпретатора Пролога. Это место уже не раз упоминалось в нашей истории.   
Там Дэвид Плейстед (David Plaisted) в 82-83гг. имплементировал на основе OBJT новую версию - OBJ1. Которую разработали Гоген и Хосе Месегер (Jose Meseguer). OBJ1 также имплементировали в университетах Манчестера и Милана на Паскале и C.   
OBJ2 в 1983-84 [Megr88] [Futa20] или в 84-85 [Goug88a] годах имплементировали Футацуги Кокити (Futatsugi Kokichi) и Жан-Пьер Жаннад (Jean-Pierre Jouannaud). В разработке этой версии языка участвовали помимо имплементаторов еще Гоген и Месегер. В этой версии уже были параметризованные модули, теории и "views", но пока не настолько обобщенные, как планировалось в CLEAR.   
Имплементацию OBJ2 также написали у Университете штата Вашингтон на Franz LISP. И собирались имплементировать на основе CLEAN разработчики CLEAN, да так и не собрались.   
Более полно осуществить планы Гогена по параметризации удалось в следующей версии. OBJ3 написали в 1985-87 Тимоти Уинклер (Timothy Winkler), Месегер, Гоген, Клод и Элен Киршнеры (Claude и Helene Kirchner) и Аристид Мегрелис (Aristide Megrelis). OBJ3 был написан на, как мы выяснили, единственной доступной в это время свободной имплементации Common Lisp - (A)KCL [Megr88].   
Как и О'Доннел, Гоген считал, что языки алгебраической спецификации родственны функциональным языкам уравнений с ПМ и перечисляет такие языки как Hope, Miranda, Standard и не только ML, а также Haskell. Гоген ссылается на TR666 88-го года, следит за новинками ФП. Но не смотря на сходные предпосылки, Гоген выбирал другие решения, развивая OBJ. Не те, что выбирали авторы языков Эдинбургской программы.   
Все эти годы Гоген работал над параметризацией, но не той, что в HOPE. Если помните, во времена разработки CLEAR и HOPE выбирали что параметризовать: типы, модули или может и то и другое. В HOPE в результате параметризовали типы, и планировали еще и модули, пока не перестали планировать. В OBJ, как и в Algol 84 и Scratchpad 2, выбрали только модули.   
Но Гоген решил, что параметризованные модули позволяют не только не добавлять в язык параметризацию типов. Можно не добавлять в OBJ еще кое что, что эдинбуржцы добавили в NPL, чтоб получить HOPE и Standard ML. В 88-ом он опубликовал статью [Goug88] под названием "Функции высшего порядка для программирования высшего порядка не нужны". И в статье демонстрирует, как без ФВП можно обойтись:
...
make TEST1 is MAP[(add1).FNS] endm

reduce map(1 2 3) .    
```

Обратите внимание на реально существующую "перегрузку оператора пробел".   
Пример проще, чем реальная имплементация `map` потому, что этот "map" не может поменять тип элементов в списке. Но даже если посмотреть на эту упрощенную версию, вполне можно представить, что кто-то решит, что для программирования высшего порядка, поддержка в языке функций высшего порядка может быть не бесполезной.   
И хотя основные, обычные языки алгебраической спецификации не стали функциональными, нашлись желающие создать функциональный язык алгебраической спецификации.

#### OPAL

В Берлинском техническом университете (Technische Universität Berlin), начиная с 1989-го года [SEQSIGN] Готфрид Эггер (Gottfried Egger), Андреас Фетт (Andreas Fett), Карола Герке (Carola Gerke), Вольфганг Грискамп (Wolfgang Grieskamp), Михаэль Яцек (Michael Jatzeck), Петер Пеппер (Peter Pepper), и Вольфрам Шульте (Wolfram Schulte) разрабатывали язык OPAL. Авторы OPAL описывают его как "современный" функциональный язык, такой как HOPE, ML, Miranda и Haskell. Но одновременно и язык алгебраической спецификации как CIP-L (Algol 84) и OBJ. [Schu92] [Didr94] [Exne94] [OPAL16]   
На самом деле не совсем такой как ML, потому что чисто функциональный. И не совсем такой как Miranda и Haskell потому, что энергичный. Но зато и то и другое как в HOPE! Авторы еще называют язык "аппликативным", как Бурсталл называл такие языки.   
Сходство с Algol 84 и OBJ, правда, означает, что у OPAL есть и принципиальное отличие от функциональных языков Эдинбургской программы. Посмотрите на определение списка в библиотеке [SEQSIGN] : 
...
У алгебраического типа нет параметра. Но параметр есть у модуля. Так что с помощью другого модуля с большим числом параметров можно написать функцию `map`. Библиотечная [SeqImpl] выглядит так:   
...
В OPAL нет параметрического полиморфизма и вывода типов по Хиндли-Милнеру, как в языках Эдинбургской программы. Вместо них нужно использовать средства языков алгебраической спецификации [Schu92] [Didr94].
Последний шаг, необходимый для HOPE-фикации не был сделан. Но из всех языков, HOPE-фикация которых не была завершена, едва ли есть более похожий на HOPE язык, чем OPAL. Как и в стандартной библиотеке эдинбургской версии HOPE функция `map` называется `*`. И в OPAL работает перегрузка, примерно как в HOPE. Как и в HOPE функции часто не каррированы, но в отличие от тех версий HOPE, про которые мы уже писали, есть и способ легковесного объявления каррированных функций. Но не настолько легковесный как в большинстве языков Эдинбургской программы.   

```
DEF f(a,b)(c) == expr
```

Лямбды же легковесного карринга не поддерживают, их нужно вкладывать друг в друга, как в HOPE:
 
```
DEF f == \\a,b. \\c. expr   
```

Паттерн матчинг в OPAL тоже больше похож на тот, что в HOPE, сопоставление зависит не от порядка паттернов, а от их "специализации". Как и в HOPE, есть `AS` паттерны. Нет настоящих гард, вместо них `IF`-выражения. Не понятно, почему библиотечная функция `map` реализована на них, можно было написать
...
и многие функции работы со списками так и написаны.   
Помните Scratchpad 2 в котором ПМ в уравнениях работает практически только для чисел? В OPAL наоборот, для чисел кроме `0` он не работает. Только `0` является конструктором.   
Есть и `WHERE` и `LET` выражения

```
map(\\x. x + y, 1 :: 2 :: 3 :: <>) WHERE y == 1  
```

С 89 года вышли OPAL-0 [Didr94], OPAL-1 и весной 94-го - OPAL 2.1 [Exne94] в котором были добавлены еще несколько фич, похожих на те, что в HOPE. `_`-вайлдкарды для ПМ и сечения. Как в лондонском HOPE, которые более громоздкие, чем сечения из Orwell, но зато работающие и с функциями:
...
Это часть нашей истории про то, как языки делали похожими на HOPE, но из HOPE обычно заимствовали только основные хорошие идеи, а не столько мелких, часто раздражающих деталей.   
В начале 80-х язык, объединяющий CLEAR и HOPE, вполне мог состояться. Так что OPAL - еще один вариант того, как мог бы выглядеть "обычный ФЯ", если бы не победа вывода типов по Хиндли-Милнеру в рамках Эдинбургской программы.   

### Пустой список

Лисп - важное средство имплементации ФЯ. И мы уже рассказывали и еще расскажем про имплементации HOPE-фицированных ФЯ добавлением соответствующего фронтенда к компилятору Лиспа или Схемы. Так что HOPE-фикации Лиспов состоялись примерно таким же способом, как HOPE-фикация Poly.   
Но что с другим способом HOPE-фикации, когда в функциональный язык добавляются фичи, пока он не становится HOPE? Как в Lazy или Standard ML. Функциональные Лиспы появились в конце 70-х и стали более-менее обычными в 80-е. Что насчет Лиспов с параметрическим полиморфизмом, АлгТД и разбирающими их уравнениями с ПМ?   
Эта часть истории началась с отчета Стила [Stee82b] в котором он называет HOPE Лиспом и включает в список Лиспов. Но Стил и прочие лисперы сами такие Лиспы делать не спешили.    
Со временем у них дошли руки и до таких Лиспов, но не скоро. Такие Лиспы достаточно современное явление.   
HOPE-фикация была поиском функционального языка современного вида. Даже там, где она не была в итоге успешна и затянулась, как в случае языков переписывания по правилам, она была естественным продолжением исследовательской программы, которая на каком-то этапе была передовой и откуда авторы ФЯ современного вида черпали идеи. И со времен рубежа 60-х и 70-х и паттерн-матчинга МакБрайда это история не про Лисп.   
Создание HOPE-фицированных Лиспов началось, когда облик современного ФЯ уже определился и чаще было попыткой добавить в современные ФЯ Лисп-образную систему мета-программирования.   
Большинство таких Лиспов появились между серединой двухтысячных и серединой десятых годов. Их неполный перечень включает Qi и Shen [Tarv15]
...
Некоторые из них, например Shen - более экспериментальные и отличающиеся от HOPE-фицированных ФЯ Эдинбургской программы. А некоторые менее отличающиеся. Так Liskell - это просто Лисп-образный синтаксис для GHC-Haskell. Но эта история HOPE-фикации Лиспа в двадцать первом веке слабо связана с нашей историей и ждет своего рассказчика.   
Нельзя сказать, что ничем даже отдаленно похожим не занимались в описываемые нами времена HOPE-фикации. Но занимались очень мало. Так датчане в своих экспериментах [Jorg90] с частичным вычислением произвели что-то вроде HOPE-фицированного Лиспа 
...
Но, как вы помните, конечный результат был довольно обычно выглядящей "Мирандой" под названием BAWL.   
Имплементаторы T Рис и Келси в 86-ом году создали Pre-Scheme [Kels97] [What24] - статически типизируемое подмножество Scheme для написания низкоуровневого кода. Статические типы там выводились по Хиндли-Милнеру. Но дальше этого дело не пошло.   
Почему лисперы не хотели HOPE-фицировать Лисп в 80-е? Мы полагаем, что у этого было две причины. Во-первых, добавлять систему типов похожую на ту, что в ML в готовый "динамический" язык без типов не легко. Это было проблемой не только для лисперов и мы еще вернемся к таким проблемам в этой части. Во-вторых, лисперов все еще не особенно интересовало функциональное программирование. О том, каким программированием они считали нужным интересоваться в это время мы расскажем в следующей части.   
В этой же части нам остается рассказать только о другой великой исследовательской программе.   

### Чао, Пролог

Логическое программирование - одно из ближайших направлений к Эдинбургской программе и даже было частью этой программы, отколовшись в середине семидесятых годов. Возможно, правильнее сказать, что откололась как раз программа создания HOPE-образных функциональных языков. Возможно, что услышав "Эдинбургская исследовательская программа", декларативный программист 80-х подумал бы в первую очередь о Прологе.   
Но не смотря на идейную и географическую близость исследовательских программ, не смотря на успехи Уоррена по созданию компилятора, не смотря на развитость и богатство направления, еще выросшее после начала японской программы по созданию компьютера пятого поколения, не смотря на все это имплементации Пролога не играли заметной роли в нашей истории после завершения нулевой части. В каталоге первых компиляторов функциональных языков не было компиляторов, использующих имплементацию Пролога как основу. Почему? Давайте попробуем разгадать эту загадку.   

#### Quintus Prolog

Компилятор Уоррена, как и многие другие имплементации, о которых мы писали в нулевой части, оказался в смертельной ловушке PDP-10. С одной стороны может показаться, что он оказался не в таком плохом положении, ведь был написан, в основном, не на каком-нибудь экзотическом языке, который был имплементирован только для PDP-10. Первый компилятор Пролога был написан на Прологе. Но зато сам способ имплементации Пролога - разделение структур (structure sharing), придуманный Бойером и Муром, обеспечивал хорошую производительность только на PDP-10. В основном из-за использования пар указателей называемых "молекулами", которые на PDP-10 помещались в одно слово, но не помещались в одно слово на новых 32-битных компьютерах. Нужно было изобрести новый способ имплементации языка высокого уровня и такие события случаются не часто.   
К счастью, еще в 76-ом году, в Лёвенском католическом университете (KU Leuven) Морис Брёйнюге (Maurice Bruynooghe) изобрел копирование структур (structure copying) [Körn22], способ имплементации как раз подходящий для новых машин.   
С тех пор, как мы оставили Уоррена в конце 70-х, он приобрел неудачный опыт коммерциализации первого компилятора Пролога. Неудачный не только из-за закрытия линейки PDP-10 без продолжения, но и потому, что правительство Великобритании не очень-то хотело, чтоб Уоррен зарабатывал деньги на продаже компилятора, созданного на государственные деньги. Эти проблемы, как и проблемы с PDP-10 происходили из того, что компилятор Уоррена опередил время. Разработчики компиляторов эдинбургских ФЯ обычно не успевали ничего сделать для PDP-10 и когда дело доходило до коммерциализации, как у Мэттьюза с Poly/ML, правительство уже относилось к таким вещам лучше. Но в 81-ом году Уоррену оставалось только оставить Эдинбург и отправиться работать в США в SRI.   
Там в 83-ем году компания DEC заплатила Уоррену за имплементацию Пролога, по крайней мере такую же быструю как его первый компилятор, но для VAX-11. Уоррен действительно разработал [Warr83] виртуальную машину, названную WAM (Warren Abstract Machine). Уоррен не назвал ее в честь себя, такое название придумали в Аргоннской национальной лаборатории [VRoy94]. До того как это название стало популярным, машина называлась New Prolog Engine. WAM использовала копирование структур и стала де-факто стандартом для имплементации Прологов. И продолжает им быть до сих пор.   
В 1984-ом в Пало Альто Уоррен вместе с другим разработчиком первого компилятора Пролога Перейрой основали компанию Quintus Computer Systems для разработки коммерческой имплементации эдинбургского Пролога на основе WAM. Имплементация называлась Quintus Prolog и разрабатывалась с 84-го года в первую очередь для VAX-11 и MC68K. Quintus Prolog 1.0 вышел в 1985.   
Так почему для имплементации некоторых ФЯ, о которых мы писали до сих пор, использовался Лисп, но не использовался Пролог? Между катастрофой PDP-10 и коммерциализацией Лиспов были свободные и более-менее быстрые имплементации Лиспа. Но таких не было для Пролога. Уоррен продавал свою первую же имплементацию для VAX-11 и MC68K. В 87 коммерческий компилятор Уоррена стоил $17,000 ($50K 2026) для VAX и $8400 ($25K 2026) для рабочей станции [Kula87]. Так что для желающего взять имплементацию Пролога и использовать ее как основу имплементации ФЯ ситуация была даже более тяжелая, чем для желающего использовать Лисп.   
Можно было бы посчитать исключением POPLOG, который стал имплементацией Пролога раньше, чем имплементацией Лиспа. Но подходить для имплементации SML этот компилятор стал только после имплементации Лиспа. Да и, скажем прямо, особенно важной эта имплементация SML не была.   
В 84-ом году появилась и коммерческая имплементация Пролога от Мориса Брёйнюге, но новый Уорреновский компилятор был популярнее. Был самым популярным компилятором Пролога. Пока не перестал. Победителем стала еще одна WAM-имплементация, разработчики которой вдохновлялись работами Уоррена и позднее даже купили его компанию. Не у самого Уоррена, тот продал свой стартап еще в 89 году.   

#### SICStus Prolog

Мы уже описывали множество ответов разных стран на японский вызов. Японцы создали институт ICOT, который работал над параллельным Прологом. Но в ответ обычно не создавали исследовательский институт, хотя иногда и пытались, как в Великобритании, например. И в ответ обычно не разрабатывали параллельный Пролог, а продолжали разрабатывать то, что и так уже разрабатывали. Но вот Швеция ответила японцам создав в  85-ом институт SICS (Swedish Institute of Computer Science), в котором занимались разработкой параллельных Прологов.   
Там, в лаборатории логических программных систем, Матс Карлссон (Mats Carlsson) работал над или-параллельным Прологом под руководством Саифа Хариди (Seif Haridi) [Mats12].   
В отличие от многих героев нашей истории, желавших работать над каким-нибудь специальным железом, в SICS решили разрабатывать параллельный Пролог на основе быстрой имплементации последовательного Пролога для обычных машин. И параллельный Пролог должен был работать на параллельных машинах тех времен, вроде Sequent Symmetry и Encore Multimax, от которых произошли обычные современные параллельные машины.   
И Карлссон с коллегами написали компилятор Пролога для обычных машин, который со временем стал генерировать быстрый код. Компилятор назвали SICStus Prolog.   
SICS сотрудничал с исследователями из других стран в рамках неформальных проектов, в числе прочих и с Уорреном. Эту неофициальную коллаборацию обычно называли Проект Gigalips. LIPS - логические операции в секунду. Один гига-LIPS - это много, в 87-ом году новый компилятор Уоррена это 20 KLIPS на рабочих станциях с MC68K, а какой-нибудь интерпретатор Пролога - это 1 KLIPS.   
И SICStus Prolog стал популярным в академии, вытесняя новый компилятор Уоррена. Но Карлссон изучал Пролог с помощью первого компилятора Уоррена, поэтому SICStus имплементировал эдинбургский Пролог с помощью WAM, так что дело Уоррена продолжало жить.   
Первая версия, которую распространяли - 0.3 - вышла в 86, но первой популярной была 0.5, вышедшая в 87-ом [Smit87] [VRoy94]. Эти версии были еще быстрым интерпретатором байт-кода. Компиляция в нативный код MC68K появилась в 88-ом году.   
Для версий 0.5 - 0.7 можно было получить исходный код и SICStus стал основой для или-параллельных Прологов Aurora и MUSE (не особенно важных для нашей истории) и и-параллельного Пролога, о котором ниже. Сотрудничество с SICS было, по видимому, единственным способом получить исходный код серьезного компилятора Пролога для экспериментов.   
Позднее код SICStus перестал быть доступным. Похоже, это не было сюрпризом, по ранним сообщениям в рассылках понятно, что его собирались продавать. И с 1988 по 91 годы шла коммерциализация SICStus Prolog при участии Ericsson Telecom AB и других шведских компаний. Но тем немногим, кто не побоялся использовать этот код, SICS разрешил открыть код имплементаций, происходящих от SICStus.   
SICStus Prolog продолжал быть популярным и после коммерциализации и вытеснил новый компилятор Уоррена уже не только из академии. В 93-ем году он был самой популярной имплементацией для рабочих станций [VRoy94]. В 98-ом SICS купил Quintus Prolog у британской компании, которая купила его в 89-ом у Уоррена.   
Компиляторы Уоррена и имплементации, которые использовали наработки из имплементаций Уоррена были популярны. И сделали де факто стандартным тот диалект Пролога, который Уоррен имплементировал.   
Конечно, для работы компилятора Пролога в описываемые времена был желателен мини-компьютер или дорогая рабочая станция. Так что для популярности эдинбургской версии Пролога было важно, что будет еще и интерпретатор для машин послабее. Для обучения программированию на Прологе. И наш старый знакомый Луиш Дамаш написал такой интерпретатор для EMAS, а один из авторов компилятора Уоррена - Перейра переписал EMAS Prolog в 81-ом году на C.   
Но нужно уточнить, что популярность эдинбургского Пролога была популярностью эдинбургского Пролога 70-х. В 80-е в Эдинбурге появились новые идеи развития Пролога.   Другой наш старый знакомый Алан Майкрофт и Ричард О’Киф придумали [Mycr84] для Пролога статические типы и процедуры высшего порядка:
...
HOPE-фицировать в Эдинбурге пытались не только ML, а еще и Пролог.   
Но похоже, что Уоррен не был сторонником существенных изменений Пролога. В 81-ом вышла статья [Warr81] Уоррена, в которой он задается вопросом: нужны ли расширения Пролога для поддержки высшего порядка? И смело отвечает: нет, не нужны.   
Вместо 
...
Уоррен считал процедуры высшего порядка слишком абстрактными, а значит - непонятными. Польза от них не стоит добавляемой в язык сложности. Но, в принципе, Уоррен не против расширения, которое позволит не повторять `apply`.   
Чего по мнению Уоррена в Прологе не хватает - так это ЦФ-нотации или какого другого удобного способа добираться не только до `x` в `x <- xs`, но и до `xs`. Что, похоже, является довольно неожиданной победой анти-Прологов из МТИ.   
И судя по тому, как развивался имплементируемый Уорреном Пролог, HOPE-фикацией Пролога Уоррен так и не заинтересовался. Не помогло и то, что Ричард О'Киф пришел работать в его стартап.   
И стандарт 95-го года зафиксировал, по большей части, именно этот уорреновский эдинбургский пролог конца 70-х.   
Но не все были настолько консервативны, а краткость кода была важным аргументом для сторонников Пролога и если писать на нем более-менее обычные функции, подход, который сам же Уоррен и сделал популярным, то Пролог получается не таким уж кратким.   
Поэтому появились нестандартные функциональные расширения Пролога.    

#### Ciao Prolog

В Университете Техаса в Остине занимались и-параллельным Прологом с 83-го года [Körn22].  В разработке участвовал Уоррен, но не тот самый, а Ричард Уоррен. Когда был организован один из американских ответов японцам - MCC, эта организация подключилась к разработке и-параллельного Пролога. А когда все эти разработчики подключились к проекту Gigalips - они получили ранние исходники SICStus [Herm91] и с 86-го (или 87-го [Demo12]) использовали их для имплементации и-параллельного Пролога. Имплементация так и называлась - &-Prolog. Наиболее важным для нашей истории разработчиком этого Пролога был Мануэль Эрменехильдо.   
Мануэль Эрменехильдо (Manuel V. Hermenegildo) защитил диссертацию в университете Техаса в Остине в 86-ом и работал до декабря 89-го в MCC. А в январе 90-го стал руководителем группы CLIP в Мадридском политехническом университете [Herm25]. Эта группа, финансируемая в рамках проекта ACCLAIM, продолжала работу над &-Prolog, который с 93-го назывался CIAO Prolog.   
Этот компилятор Пролога был и и-параллельным и или-параллельным. Но для нашей истории важно то, что его исходный код открыт и он специально разрабатывался для облегчения добавления расширений в Пролог и был для этого использован.   
Первый релиз этого первого серьезного компилятора Пролога с открытым кодом состоялся только в феврале 97-го года [Demo12] [Buen25].   
И уже в следующем релизе 0.2 в апреле 97-го имплементируемый им Пролог был расширен функциональным синтаксисом. И полной поддержкой предикатов высшего порядка не позднее версии 0.8 от октября 98-го. Версия 1.0 была готова летом 99-го.   
Функциональный синтаксис вылядел так [Herm2000] [Casa2006] [Buen25] :   
...
Эта функциональная нотация дополнялась системой аннотации и объявления типов данных

```
list_of(T) := [] | [~T | list_of(T)].  
```

Но это были эксперименты по добавлению типов в уже существующий язык без типов - Пролог , а не обычный параметрический полиморфизм и вывод типа по Хиндли-Милнеру. Так что полной HOPE-фикации не получилось. Авторы пишут [Herm08], что хотели сохранить Пролог языком для прототипирования "вроде Схемы или Питона", не мешать программисту всякими обязательными типами.   
Ciao не был единственным серьезным компилятором Пролога с открытым исходным кодом. Примерно в то же время, в апреле 99-го появился GNU Prolog [Diaz2000] [Diaz12]. В INRIA тоже достали код SICStus, но, посмотрев на него, решили не связываться и в течении 90-х годов написали компилятор с нуля. Эта история предлагает еще одно объяснение для не особенно частого использования компиляторов Пролога для имплементации языков.   
Конечно, в конце 90-х было достаточно более подходящих компиляторов. Для того, чтоб стать основой имплементации функционального языка. Свободные имплементации Пролога опоздали на полтора-два десятка лет, чтоб попасть в наш каталог первых компиляторов.   
Ciao не был и единственным Прологом, для которого имплементировали функциональные расширения. Но в Ciao они, по видимому, были самой успешной и серьезной имплементацией.   
В 95-2003 годах Тибор Ашваньи (Ásványi Tibor) разрабатывал функциональное расширение для SICStus Prolog, последовательно называвшееся fnProlog [Ásvá98], Nar-Prolog [Ásvá99] и PLN [Ásvá2003]. Второе и третье означает "Prolog with narrowing" и указывает на способ интеграции функциональных фич в логический язык.   

```
map([X|Xs],F) = [apply(F,X) | map(Xs,F)].
map([],_) = [].

map([1,2,3],^(+(Y))).
```

Ашваньи не ограничивался расширением SICStus и предлагал добавить функциональный синтаксис и в стандартный Пролог. Такие предложения появлялись и позднее [Rodr2008]
...
Может быть когда-нибудь стандартный Пролог еще станет функциональным языком. Особо рассчитывать на это мы бы не советовали, конечно.   
Но если между Прологом и условным функциональным языком уравнений никогда не было непреодолимой пропасти, то с превращением Пролога именно в очередной HOPE все сложнее из-за системы типов.   
Если пытаться оставить Пролог Прологом не смотря ни на что, то его HOPE-фикацию до конца не довести. Но не все хотели сохранить Пролог Прологом. И мало какие авторы произошедших от Пролога языков были готовы не сохранить так много Пролога, как авторы самого популярного языка, произошедшего от Пролога.  

ПРОДОЛЖЕНИЕ СЛЕДУЕТ

[Поддержать проект](https://boosty.to/fphistory)

[Перевод исправленной и дополненной нулевой части на английский](https://klapaucius.github.io/fphistory/)

Литература
==========

[Tarv15]: M. Tarver. Shen Programming Language https://github.com/Shen-Language   
[Fruh2007]: Fruhwirth, Clemens. "Haskell Semantics with Lisp Syntax."    
[Rush13]: Sam Rushing, The Irken Language https://dark.nightmare.com/rushing/irken/irken/lang.html   
[Egi11]: Egi, Satoshi. The Egison Programming Language. 2011. https://www.egison.org   
[King17]: Alexis King, Hackett https://github.com/lexi-lambda/hackett   
[Kels97]: Kelsey, Richard A. "Pre-Scheme: A Scheme dialect for systems programming." (1997).   
[What24]: Andrew Whatson. Pre-Scheme https://prescheme.org/   
[Diaz2000]: Diaz, Daniel, and Philippe Codognet. "GNU Prolog: beyond compiling Prolog to C." In International Symposium on Practical Aspects of Declarative Languages, pp. 81-92. Berlin, Heidelberg: Springer Berlin Heidelberg, 2000.   
[Diaz12]: Diaz, Daniel, Salvador Abreu, and Philippe Codognet. "On the implementation of GNU Prolog." Theory and Practice of Logic Programming 12, no. 1-2 (2012): 253-282.   
[Ásvá98]: Ásványi, T. "Functions in Full Prolog." Annales Univ. Sci. Bud. Sec. Comp (1998).   
[Ásvá99]: Ásványi, Tibor. "Adding functions to SICStus prolog." (1999).   
[Ásvá2003]: Ásványi Tibor. "User's functions in standard Prolog." (2003).   
[Rodr2008]: Rodríguez-Hortalá, Juan, and Jaime Sánchez-Hernández. "Functions and lazy evaluation in prolog." Electronic Notes in Theoretical Computer Science 206 (2008): 153-174.   
[Mycr84]: Mycroft, Alan, and Richard A. O'Keefe. "A polymorphic type system for Prolog." Artificial intelligence 23, no. 3 (1984): 295-307.   
[Herm08]: Hermenegildo, Manuel V., Francisco Bueno, Manuel Carro, Pedro López, José F. Morales, and German Puebla. "An overview of the ciao multiparadigm language and program development environment and its design philosophy." Concurrency, Graphs and Models: Essays Dedicated to Ugo Montanari on the Occasion of His 65th Birthday (2008): 209-237.   
[Herm25]: Manuel Hermenegildo, CV https://cliplab.org/~herme/engcur/engcur.pdf   
[Arms97]: Joe Armstrong, Erlang 4.4 announcement, April 16, 1997 https://groups.google.com/g/comp.lang.functional/c/I0w6dTycIos   
[Arms90]: Armstrong, J.L., and S.R. Virding. “ERLANG - an Experimental Telephony Programming Language.” International Symposium on Switching 3 (n.d.): 43–48. https://doi.org/10.1109/ISS.1990.765711     
[Warr83]: Warren, D. H. (1983). An abstract Prolog instruction set. Technical report.   
[Demo12]: Demoen, Bart, De La Banda, and Maria Garcia. "Introduction to the special issue on Prolog systems." Theory and Practice of Logic Programming 12, no. 1-2 (2012): 1-4.       
[Mats12]: Carlsson, Mats, and Per Mildner. "SICStus Prolog—the first 25 years." Theory and Practice of Logic Programming 12, no. 1-2 (2012): 35-66.   
[Buen25]: Francisco Bueno, Manuel Carro, Manuel Hermenegildo, Pedro López, José F. Morales. The Ciao System, Version 1.25 (2025/6/10) https://ciao-lang.org/ciao/build/doc/ciao.html/ciao.html    
[Casa2006]: Casas, Amadeo, Daniel Cabeza, and Manuel V. Hermenegildo. "A syntactic approach to combining functional notation, lazy evaluation, and higher-order in LP systems." In International Symposium on Functional and Logic Programming, pp. 146-162. Berlin, Heidelberg: Springer Berlin Heidelberg, 2006.   
[Herm2000]: Hermenegildo, Manuel V., Francisco Bueno Carrillo, Manuel Carro Liñares, Alvaro Germán Puebla Sánchez, Pedro López García, and Daniel Cabeza Gras. "The ciao logic programming environment: A tutorial." (2000).   
[Herm91]: Hermenegildo, Manuel V., and K. J. Greene. "The &-prolog system: Exploiting independent and-parallelism." New Generation Computing 9, no. 3 (1991): 233-256.   
[Körn22]: Körner, Philipp, Michael Leuschel, João Barbosa, Vítor Santos Costa, Verónica Dahl, Manuel V. Hermenegildo, Jose F. Morales et al. "Fifty years of Prolog and beyond." Theory and Practice of Logic Programming 22, no. 6 (2022): 776-858.   
...
[Hari88]: Seif Haridi, Announcing SICStus Prolog version 0.6, Apr. 1988 https://groups.google.com/g/comp.lang.prolog/c/O75y-0ohTC0/m/0jWQMLK_QcgJ   
[Smit87]: Bruce Smith, LIST-OF-PROLOGs, Dec. 87 https://groups.google.com/g/comp.lang.prolog/c/MSdSY-cbwEU/m/fkFEwNcwq-AJ   
[VRoy94]: Van Roy, Peter. "1983–1993: The wonder years of sequential Prolog implementation." The Journal of Logic Programming 19 (1994): 385-441.   
[Kula87]: Rich Kulawiec, List of Available Prologs, March 1987 https://groups.google.com/g/comp.lang.prolog/c/L19KVtY1Pzg/m/AKgXb5zkBGIJ   
...
[Stra94]: Metzemakers, Timo, Alain Miniussi, David Sherman, and Robert Strandh. "Improving arithmetic performance using fine-grain unfolding." In International Symposium on Programming Language Implementation and Logic Programming, pp. 324-339. Berlin, Heidelberg: Springer Berlin Heidelberg, 1994.   
[O'Do98]: O’Donnell, Michael J. "Equational logic programming." Handbook of Logic in Artificial Intelligence and Logic Programming 5 (1998): 69-161.   
[Gara18]: Hubert Garavel, Mohammad-Ali Tabikh, Imad-Seddik Arrada. Benchmarking Implementations of Term Rewriting and Pattern Matching in Algebraic, Functional, and Object-Oriented Languages - The 4th Rewrite Engines Competition. Proceedings of the 12th International Workshop on Rewriting Logic and its Applications (WRLA'18), 2018, Thessaloniki, Greece.   
[SDFMAP]: Map.sdf https://raw.githubusercontent.com/cwi-swat/meta-environment/8e234084f00b8405a4bd4c75e91bc3ad09cc1ea2/asf-library/library/functions/Map.sdf   
[ASFMAP]: Map.asf https://github.com/cwi-swat/meta-environment/blob/8e234084f00b8405a4bd4c75e91bc3ad09cc1ea2/asf-library/library/functions/Map.asf   
[STRMAP]: common.str https://github.com/metaborg/strategoxt/blob/96a3fb2d78b90da8c238e1a569a9dcab89d43379/strategoxt/stratego-libraries/lib/spec/collection/list/common.str#L105   
[Viss99]: Visser, Eelco. "Strategic pattern matching." In International Conference on Rewriting Techniques and Applications, pp. 30-44. Berlin, Heidelberg: Springer Berlin Heidelberg, 1999   
[Berg89]: Bergstra, Jan A., Jan Heering, and Paul Klint. "The algebraic specification formalism ASF." (1989): 1-66.   
[Heer89]: Heering, Jan, Paul Robert Hendrik Hendriks, Paul Klint, and Jan Rekers. "The syntax definition formalism sdf—reference manual—." ACM Sigplan Notices 24, no. 11 (1989): 43-75.   
[Viss99]: Visser, Eelco. "Strategic pattern matching." In International Conference on Rewriting Techniques and Applications, pp. 30-44. Berlin, Heidelberg: Springer Berlin Heidelberg, 1999   
[Bos11]: Bos, Jeroen van den, Mark Hills, Paul Klint, Tijs Van Der Storm, and Jurgen J. Vinju. "Rascal: From algebraic specification to meta-programming." arXiv preprint arXiv:1107.0064 (2011).   
[Vinj13]: Jurgen Vinju, From functions to term rewriting and back in Rascal https://www.rascal-mpl.org/blog/2013/05/15/from-functions-to-term-rewriting-and-back/    
[LISTRSC]: List.rsc https://github.com/usethesource/rascal/blob/main/src/org/rascalmpl/library/List.rsc   
[Grae91]: A. Gräf. Left-to-right tree pattern matching. In Proceedings Rewriting Techniques and Applications ’91, LNCS 488, pages 323-334. Springer, Berlin, 1991.   
[QNEWS]: Q NEWS https://web.archive.org/web/20040220222330/http://www.musikwissenschaft.uni-mainz.de/~ag/q/NEWS   
[Graef94]: Q - applicative programming language V1.7 https://groups.google.com/g/alt.sources/c/o_ZlYF5Cwqw/m/Yc1esZdB12YJ   
[Grae21]: Gräf, Albert. "Pure Language and Library Documentation." (2021).   
[Grae08]: Q - Equational Programming Language https://q-lang.sourceforge.net/   
...
[Megr88]: Mégrelis, Aristide. "OBJ: Programming with Equalities, Subsorts, Overloading and Parameterization." In Algebraic and Logic Programming: International Workshop, Gaussig, GDR, November 14-18, 1988. Proceedings, p. 41. Springer, 2005.   
[Goug88a]: Joseph Goguen, Claude Kirchner, Hélène Kirchner, Aristide Mégrelis, José Meseguer, and Timothy Winkler. 1988. An introduction to OBJ 3. In Lecture Notes in Computer Science. Springer Berlin Heidelberg, 258–263. doi:10.1007/3-540-19242-5_22   
[Goug88]: Goguen, Joseph. Higher order functions considered unnecessary for higher order programming. SRI International, Computer Science Laboratory, 1988.   
[Futa20]: Professor Kokichi FUTATSUGI https://www.jaist.ac.jp/~kokichi/   
[OPAL16]: An Overview of the Opal Language https://web.archive.org/web/20160317033949/http://projects.uebb.tu-berlin.de/opal/trac/wiki/Language   
[SEQSIGN]: Seq.sign https://git.tu-berlin.de/github-TU-Berlin/opal/-/blob/master/src/lib/AggregateTypes/SeqLike/Seq.sign   
[SeqImpl]: SeqMap.impl https://git.tu-berlin.de/github-TU-Berlin/opal/-/blob/master/src/lib/AggregateTypes/SeqLike/SeqMap.impl   
[Schu92]: Schulte W., Grieskamp W. (1992) Generating Efficient Portable Code for a Strict Applicative Language. In: Darlington J., Dietrich R. (eds) Declarative Programming, Sasbachwalden 1991. Workshops in Computing. Springer, London. https://doi.org/10.1007/978-1-4471-3794-8_16   
[Didr94]: Didrich, Klaus, Andreas Fett, Carola Gerke, Wolfgang Grieskamp, and Peter Pepper. "OPAL: Design and implementation of an algebraic programming language." In Programming Languages and System Architectures: International Conference Zurich, Switzerland, March 2–4, 1994 Proceedings, pp. 228-244. Berlin, Heidelberg: Springer Berlin Heidelberg, 2005.   
[Exne94]: Exner, Jürgen. The OPAL tutorial. Technische Universität Berlin, Fachbereich 13, Informatik, 1994.   
[Burg87]: Burge, William H., and Stephen M. Watt. "Infinite structures in Scratchpad II." In European Conference on Computer Algebra, pp. 138-148. Berlin, Heidelberg: Springer Berlin Heidelberg, 1987.   
[Spad87]: Scratchpad II System Availability, The Scratchpad II Newsletter: Vol. 2 Num. 1   
[ListSpad]: List https://github.com/fricas/fricas/blob/master/src/algebra/list.spad   
[Aldor2002] aldor.org, Aldor User Guide, 2002   
[Jenk86]: Richard D. Jenks, A History of the SCRATCHPAD Project (1977-1986)   
[Daly2003]: Tim Daly, GCL used commercially, 23 Jul 2003 https://lists.nongnu.org/archive/html/axiom-developer/2003-07/msg00114.html    
[Jenk94]: Jenks, Richard D., and Barry M. Trager. "How to Make AXIOM into a Scratchpad." In Proceedings of the international symposium on Symbolic and algebraic computation, pp. 32-40. 1994.   
[Page2005]: Bill Page, B#, 19 Nov 2005 https://lists.nongnu.org/archive/html/axiom-developer/2005-11/msg00411.html   
[Bake26]: Martin John Baker, Computer Algebra Programs. https://www.euclideanspace.com/prog/scratchpad/   
[Daly2003]: Bronstein, Manuel, William Burge, Timothy Daly, James Davenport, Michael Dewar Martin Dunstan, Albrecht Fortenbacher Patrizia Gianni Johannes Grabmeier, Jocelyn Guidry et al. "The 30 Year Horizon."   
[Baue87]: F. L. Bauer, H. Ehler, A. Horsch, B. Möller, H. Partsch, O. Paukner, P. Pepper (eds.) The Munich Project CIP : Volume II: The Program Transformation System CIP-S, Springer-Verlag Berlin Heidelberg, Lecture Notes in Computer Science, Lecture Notes in Computer Science 292, 2, 1, 1987   
[Bran95]: van den Brand, M. G. J., S. M. Eijkelkamp, D. K. A. Geluk, H. Meijer, H. R. Osborne, and M. J. F. Polling. "Program transformations using ASF+ SDF." In ASF+ SDF, vol. 95, pp. 29-52. 1995.   
[Baue85]: F. L. Bauer, R. Berghammer, W. Dosch, R. Gnatz, E. Hangel, B. Möller, H. Partsch, P. Pepper, K. Samelson, H. Wössner, M. Broy, F. Nickl, M. Wirsing, F. Geiselbrechtinger, W. Hesse, B. Krieg-Brückner, A. Laut, T. Matzner (auth.) The Munich Project CIP Volume I:The Wide Spectrum Language CIP-L Springer-Verlag Berlin Heidelberg, Lecture Notes in Computer Science, Lecture Notes in Computer Science 183, 1, 1, 1985      
[Stoy85]: Stoye, William. “The implementation of functional languages using custom hardware.” (1985).   
[Augu25]: Haskell Interlude 74: Lennart Augustsson https://haskell.foundation/podcast/74/    
...
[Wadl25]: Philip Wadler, Haskell equations, thirty-eight years later https://wadler.blogspot.com/2025/09/haskell-equations-thirty-eight-years.html   
[Fair06]: Jon Fairbairn, comment about fplangc https://mailman.haskell.org/archives/list/haskell-cafe@haskell.org/message/FZ44BZAP2P52W2JUUOJAIAOJWLGO4DUM/   
[Huda91a]: Paul Hudak, = and ==, haskell archives 1990-2000 4 Jun 91   
[Huda90a]: Paul Hudak, PATTERN GUARDS haskell archives 1990-2000 8 Oct 90   
[Hamm90a]: Kevin Hammond, Why were sections not included in Haskell? haskell archives 1990-2000 22 Oct 90    
[Karl90a]: Kent Karlsson, Scope of 'where' haskell archives 1990-2000 27 Sep 90     
[Kewl06]: John Kewley, Talk:History of Haskell/Version 2 https://wiki.haskell.org/index.php?title=Talk:History_of_Haskell/Version_2
[Huda88]: Paul Hudak, YALEU/DCS/TR666 Report on the Functional Programming Language Haskell December 1988   
[Huda88a]: Hudak, Paul, and Steve Anderson. "Haskell Solutions to the Language Session Problems at the 1988 Salishan High-Speed Computing Conference." (1988).   
...
[Fase91a]: Joe Fasel, Re: expressions, patterns, lefthand sides (SYNTAX ONLY) haskell archives 1990-2000 5 Apr 91   
[Hamm91]: Kevin Hammond, Re: WHERE oh where has my little LET gone... haskell archives 1990-2000 26 Apr 91   
[Fase91]: Joe Fasel, Re: WHERE oh where has my little LET gone... haskell archives 1990-2000 27 Apr 91   
[SPJ91]: Simon Peyton Jones, Re: WHERE to now? haskell archives 1990-2000 8 May 91   
[SPJ91a]: Simon Peyton Jones, Revised Haskell haskell archives 1990-2000 1 Aug 91      
[SPJ91b]: Simon Peyton Jones, Authorship haskell archives 1990-2000 10 Jun 91      
[Huda90]: P. Hudak and P. Wadler (editors). Report on the Programming Language Haskell, A Non-strict Purely Functional Language (Version 1.0). Technical report, University of Glasgow, April 1990.   
[Huda91]: P. Hudak, P. Wadler, SP Jones (editors). Report on the Programming Language Haskell, A Non-strict Purely Functional Language (Version 1.1). Technical report, University of Glasgow, August 1991.   
[Hart96]: Hartel PH, Feeley M, Alt M, Augustsson L, Baumann P, Beemster M, Chailloux E, Flood CH, Grieskamp W, Van Groningen JH, Hammond K. Benchmarking implementations of functional languages with ‘Pseudoknot’, a float-intensive benchmark. Journal of functional programming. 1996 Jul;6(4):621-55.   
[MN]: About the name Miranda https://www.cs.kent.ac.uk/people/staff/dat/miranda/name.html       
[Bond90]: Bondorf, Anders. "Compiling Laziness by Partial Evaluation." In Functional Programming, pp. 9-22. 1990.   
[Frad91]: Pascal Fradet and Daniel Le Métayer. 1991. Compilation of functional languages by program transformation. ACM Trans. Program. Lang. Syst. 13, 1 (Jan. 1991), 21–51. doi:10.1145/114005.102805   
...
[Hart93]: Hartel, Pieter H., Hugh Glaser, and John Wild. "FAST compiler user's guide." In FAST: Functional Programming for Arrays of Transputers. 1993.   
[FAST96]: FAST https://web-archive.southampton.ac.uk/www.dsse.ecs.soton.ac.uk-1996/fast.html   
[Feel94]: Marc Feeley, Marcel Turcotte, and Guy Lapalme. 1994. Using Multilisp for solving constraint satisfaction problems: an application to nucleic acid 3D structure determination. Lisp Symb. Comput. 7, 2–3 (June 1994), 231–247. https://doi.org/10.1007/BF01018696    
[RSL]: Research Software Limited https://find-and-update.company-information.service.gov.uk/company/01714265   
[Plas93]: Plasmeijer, Rinus, Marko Van Eekelen, and M. J. Plasmeijer. Functional programming and parallel graph rewriting. Vol. 857. Reading: Addison-wesley, 1993.   
[Blos89]: Adrienne Bloss, P. Hudak, and J. Young. 1989. An optimising compiler for a modern functional language. Comput. J. 32, 2 (April 1989), 152–161. https://doi.org/10.1093/comjnl/32.2.152   
[TMail]: T mailing list archive. http://people.csail.mit.edu/riastradh/t/tmail.tar.bz2   
[Phil88]: James Philbin. T a dialect of Scheme, 1988 https://groups.google.com/g/comp.lang.scheme/c/nFhb_GWz8KA/m/-1OD4SeZ9mgJ   
[Kran88]: Kranz, D.A. (1988).  ORBIT: An optimizing compiler for Scheme.  Ph.D. Dissertation, Yale University Technical Report #632.   
[Kran86]: Kranz, D.A., Kelsey, R., Rees, J.A., Hudak, P., Philbin, J., and Adams, N.I. (1986).  ORBIT: An optimizing compiler for Scheme.  Proceedings of the SIGPLAN 1986 Symposium on Compiler Construction.   
...
[Jorg90]: Jorgensen, Jesper. "Generating a pattern matching compiler by partial evaluation." In Proceedings of the 1990 Glasgow Workshop on Functional Programming (Workshops in Computing), Simon L. Peyton Jones, Graham Hutton, and Carsten Kehler Holst (Eds.). Springer, 177ś, vol. 195. 1990.   
[Jorg92]: Jesper Jørgensen. 1992. Generating a compiler for a lazy language by partial evaluation. In Proceedings of the 19th ACM SIGPLAN-SIGACT symposium on Principles of programming languages (POPL '92). Association for Computing Machinery, New York, NY, USA, 258–268. doi:10.1145/143165.143220    
[Kant93]: FAQ: Scheme Frequently Asked Questions https://groups.google.com/g/comp.lang.scheme/c/RjkJo1_Q3s8/m/gQ8LYmkEjOcJ   
[Jone90]: Jones, G. (1990). Deriving the fast Fourier algorithm by calculation. In: Davis, K., Hughes, J. (eds) Functional Programming. Workshops in Computing. Springer, London. doi:10.1007/978-1-4471-3166-3_6   
[Gibb22]: Jeremy Gibbons. Richard Bird, 1943-2022 https://patternsinfp.wordpress.com/2022/06/06/richard-bird/   
[Mill90a]: Quentin Miller, comp.lang.functional Subject: Orwell Date: 17 Oct 90 11:38:57 GMT https://newsgrouper.org/%3C732@culhua.prg.ox.ac.uk%3E   
[Mill90b]: Quentin Miller, comp.lang.functional Subject: Orwell Date: 26 Nov 90 13:46:29 GMT https://newsgrouper.org/%3C934@culhua.prg.ox.ac.uk%3E   
[Bird86]: Alvey News Supplement, no. SE2/18, Alvey, 1986.   
[Bird88]: Bird, R.S., Wadler, P.: “Introduction to Functional Programming”, 293 pages, Prentice Hall 1988   
[Knut74]: Donald E. Knuth. 1974. Structured Programming with go to Statements. ACM Comput. Surv. 6, 4 (Dec. 1974), 261–301. doi:10.1145/356635.356640   
[Runc91]: Runciman C, Toyn I. Retrieving reusable software components by polymorphic type. Journal of Functional Programming. 1991;1(2):191-211. doi:10.1017/S0956796800020049   
[Wadl85]: Wadler, P. "An introduction to Orwell (draft)." (1985).   
[Wadl86]: Wadler, P., Miller, Q. and Raskovsky, M. 1986. The OL Manual. Oxford University Computing Laboratory.   
[Wadl90]: Wadler, Philip, and Quentin Miller. "Introduction to Orwell 6.00." University of Oxford (1990).   
[Wadler]: Philip Wadler, CV https://homepages.inf.ed.ac.uk/wadler/vita.pdf    
[MirMan]: Miranda system manual. http://miranda.org.uk/manual   
[MirSrc]: Miranda source code. https://codeberg.org/DATurner/miranda   
[Meir85]: Silvio Lemos Meira, A linear applicative solution for the set union problem, Information Processing Letters, Volume 20, Issue 1, 1985, Pages 43-45, ISSN 0020-0190, doi:10.1016/0020-0190(85)90128-0.   
...
[Turn85]: D. A. Turner Miranda: A Non-Strict Functional Language with Polymorphic Types, Proceedings IFIP Conference on Functional Programming Languages and Computer Architecture, Nancy, France, September 1985 (Springer Lecture Notes in Computer Science 201:1-16).   
[Turn86]: D Turner. 1986. An overview of Miranda. SIGPLAN Not. 21, 12 (Dec. 1986), 158–166. https://doi.org/10.1145/15042.15053   
